...
void* EKF_SLAM_thread(void* thread_arg);
...
void SW_EKF_SLAM::Run()
{
 pthread_t thread_id[NUM_THREADS];
  ...
  for(int p = 0; p < NUM_THREADS; p++) {
    ...
    thread1_arg[p].X = X;
    thread1_arg[p].U = U;
    thread1_arg[p].out_X_oplus_U = out_X_oplus_U;
    thread1_arg[p].out_df_dx = out_df_dx;
    thread1_arg[p].cpose = cpose;
    thread1_arg[p].point = point;
    thread1_arg[p].out_jacob_dryp_dpoint = out_jacob_dryp_dpoint;
    thread1_arg[p].out_jacob_dryp_dpose=out_jacob_dryp_dpose;
    thread1_arg[p].benchmark = this;
    pthread_create(&thread_id[p], NULL, EKF_SLAM_thread, (void *)&thread1_arg[p]);
  }
  for(int p = 0; p < NUM_THREADS; p++) {
...
void* EKF_SLAM_thread(void* thread_arg) {
  ...
  float (*X)[7] = my_thread_arg->X;
  float (*U)[7]= my_thread_arg->U;
  float (*out_X_oplus_U)[7] = my_thread_arg->out_X_oplus_U;
  float (*out_df_dx)[7*4] = my_thread_arg->out_df_dx;
  float (*cpose)[7] = my_thread_arg->cpose;
  float (*point)[3] = my_thread_arg->point;
  float (*out_jacob_dryp_dpoint)[3*3] = my_thread_arg->out_jacob_dryp_dpoint;
  float (*out_jacob_dryp_dpose)[3*7]= my_thread_arg->out_jacob_dryp_dpose;  
  SW_EKF_SLAM* benchmark = my_thread_arg->benchmark;

  int PP = dataSize/NUM_THREADS;
  for(int i = tid*PP; i < (tid+1)*PP; i++) {
    
		benchmark->jacobiansPoseComposition(X[i], U[i], out_df_dx[i], out_X_oplus_U[i]);
		benchmark->sphericalCoordinates(cpose[i], point[i], out_jacob_dryp_dpoint[i], out_jacob_dryp_dpose[i]);
	}
}
