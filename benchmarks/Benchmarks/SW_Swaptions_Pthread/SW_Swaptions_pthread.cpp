...
public:
  ...
  int  HJM_SimPath_Forward_Blocking_2(int number,int sw);
};
...
void SW_Swaptions::Run()
{
  ...
  for(int it= 0; it< ITER_COUNT; it++) {
    for(int p = 0; p < NUM_THREADS; p++) {
      ...
      thread1_arg[p].pdForward = pdForward;
...
void* Swaptions_thread(void* thread_arg) {
  ...
  FTYPE *pdForward = my_thread_arg->pdForward;
  FTYPE *pdTotalDrift=my_thread_arg->pdTotalDrift;
  FTYPE ***ppdFactors=my_thread_arg->ppdFactors;
  long lRndSeed=my_thread_arg->lRndSeed;
  FTYPE **pdZ=my_thread_arg->pdZ;
  FTYPE **randZ=my_thread_arg->randZ;
  FTYPE **factors=my_thread_arg->factors;
