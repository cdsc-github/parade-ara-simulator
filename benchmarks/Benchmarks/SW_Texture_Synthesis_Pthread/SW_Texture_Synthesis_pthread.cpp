...
void* Texture_Synthesis_thread(void* thread_arg);
...
struct thread_arg1_t {
  ...
  SW_Texture_Synthesis* benchmark;
};


void SW_Texture_Synthesis::TexSynInitVals(float in_image[HEIGHTin][WIDTHin], unsigned int in_rand_seeds[100], int in_j, int in_i, int out_atlas[2], float out_result[1])
{
#ifndef vrfinishx
...
pthread_t thread_id[NUM_THREADS];
  ...
  for(int p = 0; p < NUM_THREADS; p++) {
    ...
    thread1_arg[p].image = image;
    thread1_arg[p].target = target;
    thread1_arg[p].result = result;
    thread1_arg[p].rand_seeds = rand_seeds;
    thread1_arg[p].atlas = atlas;
    thread1_arg[p].candlistx = candlistx;
    thread1_arg[p].candlisty = candlisty;
    thread1_arg[p].ii=ii;
    thread1_arg[p].jj=jj;
    thread1_arg[p].benchmark = this;
    pthread_create(&thread_id[p], NULL, Texture_Synthesis_thread, (void *)&thread1_arg[p]);
  }
  for(int p = 0; p < NUM_THREADS; p++) {
...
void* Texture_Synthesis_thread(void* thread_arg) {
  ...
  float (*image)[HEIGHTin][WIDTHin] = my_thread_arg->image;
   float (*target)[HEIGHTout][WIDTHout] = my_thread_arg->target;
  float (*result)[HEIGHTout][WIDTHout][1] = my_thread_arg->result;
  unsigned int (*rand_seeds)[100] = my_thread_arg->rand_seeds;
  int (*atlas)[HEIGHTout][WIDTHout][2] = my_thread_arg->atlas;
  int (*candlistx)[HEIGHTout-1][WIDTHout][4]= my_thread_arg->candlistx;
   int (*candlisty)[HEIGHTout-1][WIDTHout][4] = my_thread_arg->candlisty;
  int (*ii)[HEIGHTout]= my_thread_arg->ii;
  int (*jj)[WIDTHout]=my_thread_arg->jj;
  
  SW_Texture_Synthesis* benchmark = my_thread_arg->benchmark;

  int PP = dataSize/NUM_THREADS;
  for(int d = tid*PP; d < (tid+1)*PP; d++) {

		for (int i = 0; i < HEIGHTout; i++) {
			for (int j = 0; j < WIDTHout; j++) {
				benchmark->TexSynInitVals(image[d], rand_seeds[d], jj[d][j], ii[d][i],
       ...
       for(int d = tid*PP; d < (tid+1)*PP; d++) {
		for (int i = 0; i < (HEIGHTout - 1); i++) {
			for (int j = 0; j < WIDTHout; j++) {
			benchmark->TexSynCreateCand(atlas[d], rand_seeds[d], jj[d][j], ii[d][i],
  ...
  for(int d = tid*PP; d < (tid+1)*PP; d++) {	
		for (int i = 0; i < (HEIGHTout - 1); i++) {
			for (int j = 0; j < WIDTHout; j++) {
			benchmark->TexSynCompareAll(image[d], candlistx[d][i][j], candlisty[d][i][j],
