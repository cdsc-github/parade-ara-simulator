...
#endif
#ifndef LCACC_BODY_SIG__MatMulLCacc__X
#define LCACC_BODY_SIG__MatMulLCacc__X
#define LCACC_CLASS_SIG__MatMulLCacc__mm 1
class InstanceData_sig__MatMulLCacc
{
public:
	...
	int allocatedAcceleratorCount__MatMul;
	int allocatedAcceleratorIDSet__MatMul[1];
	...
	inline void Reset()
	{
		...
		allocatedAcceleratorCount__MatMul = 0;
		reservedAccelerators = 0;
		allocatedAcceleratorIDSet__MatMul[0] = 0;
		HandlerArgs__mm.threadID = threadID;
		HandlerArgs__mm.status = 0;
		HandlerArgs__mm.taskIndex = 0;
		HandlerArgs__mm.lcaccMode = LCACC_CLASS_SIG__MatMulLCacc__mm;
		node_mm.Reset();
	}
	inline InstanceData_sig__MatMulLCacc() :
...
inline void (*GAMHandler_sig__MatMulLCacc(InstanceData_sig__MatMulLCacc* instance, InterruptArgs* args))(InstanceData_sig__MatMulLCacc*) ;
inline void Cleanup_sig__MatMulLCacc(InstanceData_sig__MatMulLCacc* instance);
inline void StartEverythingHandler_sig__MatMulLCacc(InstanceData_sig__MatMulLCacc* instance);
inline void StopEverythingHandler_sig__MatMulLCacc(InstanceData_sig__MatMulLCacc* instance);
inline void ErrorHandler_sig__MatMulLCacc(InstanceData_sig__MatMulLCacc* instance);
inline void Wait_sig__MatMulLCacc(InstanceData_sig__MatMulLCacc* instance);
inline unsigned int DelayEstimate_sig__MatMulLCacc(int vectorSize)
{
	return 1;
}
inline void (*ProgressHandler_sig__MatMulLCacc__mm(InstanceData_sig__MatMulLCacc* instance, InterruptArgs* args))(InstanceData_sig__MatMulLCacc*);
//This is a procedurally generated interrupt-based program from a RAGraph data structure for function MatMulLCacc
inline void Wait_sig__MatMulLCacc(InstanceData_sig__MatMulLCacc* instance)
{
	while(1)
	{
		...
		InterruptArgs* args = 0;
		void (*progress)(InstanceData_sig__MatMulLCacc*);
		while((args = LWI_CheckInterrupt(thread)) == 0);
		switch(args->lcaccMode)
		{
			case(0):
				progress = GAMHandler_sig__MatMulLCacc(instance, args);
				break;
			case(LCACC_CLASS_SIG__MatMulLCacc__mm):
				progress = ProgressHandler_sig__MatMulLCacc__mm(instance, args);
				break;
			default:
				ErrorHandler_sig__MatMulLCacc(instance);
				simics_assert(0);
		}
	...
	switch(args->status)
	{
		case(LCACC_GAM_WAIT):
			...
			if(instance->reservedAccelerators == 1)
			{
				...
				LCAcc_Reserve(args->threadID, 0, DelayEstimate_sig__MatMulLCacc(instance->acceleratorVectorLength));
			}
			return 0;
			break;
		case(LCACC_GAM_GRANT):
			...
			{
				case(107):
					if(instance->allocatedAcceleratorCount__MatMul == 1)
					{
						return ErrorHandler_sig__MatMulLCacc;
					}
					else
					{
						instance->pendingAccelerators++;
						instance->allocatedAcceleratorIDSet__MatMul[instance->allocatedAcceleratorCount__MatMul] = lcaccID;
						instance->allocatedAcceleratorCount__MatMul++;
					}
					break;
				default:
					return ErrorHandler_sig__MatMulLCacc;
			...
			if(instance->allocatedAcceleratorCount__MatMul == 1)
			{
				return StartEverythingHandler_sig__MatMulLCacc;
			}
			else
			{
				return 0;
...
inline void Cleanup_sig__MatMulLCacc(InstanceData_sig__MatMulLCacc* instance)
{
	...
	if(instance->allocatedAcceleratorIDSet__MatMul[0] != 0)
	{
...
inline void StopEverythingHandler_sig__MatMulLCacc(InstanceData_sig__MatMulLCacc* instance)
{
	Cleanup_sig__MatMulLCacc(instance);
}
inline void ErrorHandler_sig__MatMulLCacc(InstanceData_sig__MatMulLCacc* instance)
{
	Cleanup_sig__MatMulLCacc(instance);
...
inline void (*ProgressHandler_sig__MatMulLCacc__mm(InstanceData_sig__MatMulLCacc* instance, InterruptArgs* args))(InstanceData_sig__MatMulLCacc*)
{
	switch(args->status)
	{
		...
		case(LCACC_STATUS_COMPLETED):
			...
			{
				return StopEverythingHandler_sig__MatMulLCacc;
			}
			else
			{
				return 0;
...
inline void StartEverythingHandler_sig__MatMulLCacc(InstanceData_sig__MatMulLCacc* instance)
{
	...
	instance->HandlerArgs__mm.lcaccID = instance->allocatedAcceleratorIDSet__MatMul[0];
	...
	instance->HandlerArgs__mm.lcaccMode = LCACC_CLASS_SIG__MatMulLCacc__mm;
	...
	Wait_sig__MatMulLCacc(instance);// wait for everything to finish
}
inline void CreateBuffer_MatMulLCacc_sig(int thread, InstanceData_sig__MatMulLCacc* instance, float (*in1), float (*in2), float (*out), int sizeN, int sizeM, int sizeP)
{
	...
	instance->allocatedAcceleratorCount__MatMul = 0;
...
inline void MatMulLCacc_sig_buf(int threadID, InstanceData_sig__MatMulLCacc* instance)
{
	instance->Reset();
	LWI_RegisterInterruptHandler(&(instance->GAM_INTERACTION));
	LCAcc_Request(threadID, 107, 1);//request something of type AcceleratorType
	Wait_sig__MatMulLCacc(instance);// wait for everything to finish
}
inline void MatMulLCacc_sig(int threadID, float (*in1), float (*in2), float (*out), int sizeN, int sizeM, int sizeP)
{
	InstanceData_sig__MatMulLCacc instance;
	CreateBuffer_MatMulLCacc_sig(threadID, &instance, in1, in2, out, sizeN, sizeM, sizeP);
	MatMulLCacc_sig_buf(threadID, &instance);
}
class BiN_MatMulLCacc_Arbitrator_sig
{
	std::vector<InstanceData_sig__MatMulLCacc*> instanceSet;
	...
	InterruptArgs isr;
public:
	inline BiN_MatMulLCacc_Arbitrator_sig(int thread)
	{
		ops.push_back(107);
		count.push_back(1);
		threadID = thread;
	}
	inline void AddConfig(InstanceData_sig__MatMulLCacc* inst, uint32_t performance, uint32_t cacheMod)
	{
	...
	inline void Run()
	{
		...
		int allocatedAcceleratorCount__MatMul = 0;
		LCAcc_Request(threadID, 107, 1);//request something of type AcceleratorType
		bool cont = true;
				...
				case(LCACC_GAM_GRANT):
					{
						...
						{
							case(107):
								simics_assert(allocatedAcceleratorCount__MatMul < 1);
								allocatedAcceleratorIDSet__MatMul[allocatedAcceleratorCount__MatMul] = lcaccID;
								allocatedAcceleratorCount__MatMul++;
								break;
							default:
								simics_assert(0);
						}
						if(allocatedAcceleratorCount__MatMul == 1)
						{
							cont = false;
						}
					}
		...
		InstanceData_sig__MatMulLCacc* inst = NULL;
		for(size_t i = 0; i < instanceSet.size(); i++)
		{
		...
		for(int i = 0; i < 1; i++)
		{
			inst->allocatedAcceleratorIDSet__MatMul[i] = allocatedAcceleratorIDSet__MatMul[i];
		}
		inst->allocatedAcceleratorCount__MatMul = allocatedAcceleratorCount__MatMul;
		inst->pendingAccelerators = 1;
		StartEverythingHandler_sig__MatMulLCacc(inst);
	}
};

#endif
#ifndef LCACC_BODY_TD__MatMulLCacc__X
#define LCACC_BODY_TD__MatMulLCacc__X

inline void Wait_td__MatMulLCacc(int thread)
{
...
inline void CreateBuffer_MatMulLCacc_td(uint8_t** buffer, uint32_t* bufferSize, uint8_t** constCluster, int thread, float (*in1), float (*in2), float (*out), int sizeN, int sizeM, int sizeP)
{
	simics_assert(sizeN > 0);
	simics_assert(sizeM > 0);
	simics_assert(sizeP > 0);
	...
	void* LCAcc_FuncArgs__out = out;
	{
		...
		LCAccNode VNR_vardecl_3(107, (sizeof(float) * (1) * (1) * (sizeM)) + (sizeof(float) * (1) * (1) * (sizeM)) + (sizeof(float) * (1) * (1) * (1)), 2, 0, 0);
		std::vector<uint32_t> VNR_vardecl_4;
		VNR_vardecl_4.push_back(((1) - (0)) / (1));
		VNR_vardecl_4.push_back(((1) - (0)) / (1));
		VNR_vardecl_4.push_back(((sizeM) - (0)) / (1));
...
inline void MatMulLCacc_td_buf(uint8_t* buf, uint32_t bufSize, int thread)
{
	...
	Wait_td__MatMulLCacc(thread);// wait for everything to finish
}
void MatMulLCacc_td(int thread, float (*in1), float (*in2), float (*out), int sizeN, int sizeM, int sizeP)
{
	uint32_t bufSize;
	uint8_t* buffer;
	uint8_t* constCluster = NULL;
	CreateBuffer_MatMulLCacc_td(&buffer, &bufSize, &constCluster, thread, in1, in2, out, sizeN, sizeM, sizeP);
	MatMulLCacc_td_buf(buffer, bufSize, thread);
	if(constCluster)
	{
		delete [] constCluster;
	}
...
inline uint32_t MatMulLCacc_CalculateBiNSize(int sizeN, int sizeM, int sizeP)
{
	return 0;
}
class BiN_MatMulLCacc_Arbitrator_td
{
	...
	InterruptArgs isr;
public:
	inline BiN_MatMulLCacc_Arbitrator_td(){}
	...
	inline void Run(int threadID)
	{
		...
		isr.lcaccID = 0;
		isr.lcaccMode = 0;
		LCAcc_DeclareLCAccUse(threadID, 107, 1); //requests for MatMul
		...
		for(size_t i = 0; i < binSizeSet.size(); i++)
		{
			...
			{
				MatMulLCacc_td_buf(bufSet[i], bufSizeSet[i], threadID);
				return;
			}
		}
		simics_assert(0);
