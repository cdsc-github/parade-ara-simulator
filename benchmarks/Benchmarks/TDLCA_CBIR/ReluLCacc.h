...
#endif
#ifndef LCACC_BODY_SIG__ReluLCacc__X
#define LCACC_BODY_SIG__ReluLCacc__X
#define LCACC_CLASS_SIG__ReluLCacc__rl 1
class InstanceData_sig__ReluLCacc
{
public:
	...
	int allocatedAcceleratorCount__Relu;
	int allocatedAcceleratorIDSet__Relu[1];
	float (*LCAcc_FuncArgs__Pin);
	float (*LCAcc_FuncArgs__Pout);
	int LCAcc_FuncVars__fsize;
	int LCAcc_FuncVars__chunk;
	MicroprogramWriter acceleratorSignature__rl;
	LCAccNode node_rl;
	InterruptArgs HandlerArgs__rl;
	inline void Reset()
	{
		pendingAccelerators = 0;
		allocatedAcceleratorCount__Relu = 0;
		reservedAccelerators = 0;
		allocatedAcceleratorIDSet__Relu[0] = 0;
		HandlerArgs__rl.threadID = threadID;
		HandlerArgs__rl.status = 0;
		HandlerArgs__rl.taskIndex = 0;
		HandlerArgs__rl.lcaccMode = LCACC_CLASS_SIG__ReluLCacc__rl;
		node_rl.Reset();
	}
	inline InstanceData_sig__ReluLCacc() :
		acceleratorSignature__rl(true), 
		node_rl(246), 
		threadID(0)
	{
		Reset();
...
inline void (*GAMHandler_sig__ReluLCacc(InstanceData_sig__ReluLCacc* instance, InterruptArgs* args))(InstanceData_sig__ReluLCacc*) ;
inline void Cleanup_sig__ReluLCacc(InstanceData_sig__ReluLCacc* instance);
inline void StartEverythingHandler_sig__ReluLCacc(InstanceData_sig__ReluLCacc* instance);
inline void StopEverythingHandler_sig__ReluLCacc(InstanceData_sig__ReluLCacc* instance);
inline void ErrorHandler_sig__ReluLCacc(InstanceData_sig__ReluLCacc* instance);
inline void Wait_sig__ReluLCacc(InstanceData_sig__ReluLCacc* instance);
inline unsigned int DelayEstimate_sig__ReluLCacc(int vectorSize)
{
	return 1;
}
inline void (*ProgressHandler_sig__ReluLCacc__rl(InstanceData_sig__ReluLCacc* instance, InterruptArgs* args))(InstanceData_sig__ReluLCacc*);
//This is a procedurally generated interrupt-based program from a RAGraph data structure for function ReluLCacc
inline void Wait_sig__ReluLCacc(InstanceData_sig__ReluLCacc* instance)
{
	while(1)
	{
		...
		InterruptArgs* args = 0;
		void (*progress)(InstanceData_sig__ReluLCacc*);
		while((args = LWI_CheckInterrupt(thread)) == 0);
		switch(args->lcaccMode)
		{
			case(0):
				progress = GAMHandler_sig__ReluLCacc(instance, args);
				break;
			case(LCACC_CLASS_SIG__ReluLCacc__rl):
				progress = ProgressHandler_sig__ReluLCacc__rl(instance, args);
				break;
			default:
				ErrorHandler_sig__ReluLCacc(instance);
				simics_assert(0);
		}
	...
	switch(args->status)
	{
		case(LCACC_GAM_WAIT):
			...
			if(instance->reservedAccelerators == 1)
			{
				...
				LCAcc_Reserve(args->threadID, 0, DelayEstimate_sig__ReluLCacc(instance->acceleratorVectorLength));
			}
			return 0;
			break;
		case(LCACC_GAM_GRANT):
			...
			{
				case(246):
					if(instance->allocatedAcceleratorCount__Relu == 1)
					{
						return ErrorHandler_sig__ReluLCacc;
					}
					else
					{
						instance->pendingAccelerators++;
						instance->allocatedAcceleratorIDSet__Relu[instance->allocatedAcceleratorCount__Relu] = lcaccID;
						instance->allocatedAcceleratorCount__Relu++;
					}
					break;
				default:
					return ErrorHandler_sig__ReluLCacc;
			...
			if(instance->allocatedAcceleratorCount__Relu == 1)
			{
				return StartEverythingHandler_sig__ReluLCacc;
			}
			else
			{
				return 0;
...
inline void Cleanup_sig__ReluLCacc(InstanceData_sig__ReluLCacc* instance)
{
	...
	if(instance->allocatedAcceleratorIDSet__Relu[0] != 0)
	{
		LCAcc_Free(instance->HandlerArgs__rl.threadID, instance->HandlerArgs__rl.lcaccID);
		LWI_UnregisterInterruptHandler(instance->HandlerArgs__rl.threadID, instance->HandlerArgs__rl.lcaccID);
	}
}
inline void StopEverythingHandler_sig__ReluLCacc(InstanceData_sig__ReluLCacc* instance)
{
	Cleanup_sig__ReluLCacc(instance);
}
inline void ErrorHandler_sig__ReluLCacc(InstanceData_sig__ReluLCacc* instance)
{
	Cleanup_sig__ReluLCacc(instance);
...
inline void (*ProgressHandler_sig__ReluLCacc__rl(InstanceData_sig__ReluLCacc* instance, InterruptArgs* args))(InstanceData_sig__ReluLCacc*)
{
	switch(args->status)
	{
		...
		case(LCACC_STATUS_COMPLETED):
			...
			{
				return StopEverythingHandler_sig__ReluLCacc;
			}
			else
			{
				return 0;
...
{
	int index;
	instance->HandlerArgs__rl.threadID = instance->threadID;
	instance->HandlerArgs__rl.lcaccID = instance->allocatedAcceleratorIDSet__Relu[0];
	instance->node_rl.SetLCAccID(instance->HandlerArgs__rl.lcaccID);
	instance->HandlerArgs__rl.status = 0;
	instance->HandlerArgs__rl.taskIndex = 0;
	instance->HandlerArgs__rl.lcaccMode = LCACC_CLASS_SIG__ReluLCacc__rl;
	LWI_RegisterInterruptHandler(&(instance->HandlerArgs__rl));
	simics_assert(instance->acceleratorSignature__rl.IsFinalized());
	LCAcc_Command(instance->HandlerArgs__rl.threadID, instance->HandlerArgs__rl.lcaccID, LCACC_CMD_BEGIN_TASK_SIGNATURE, instance->acceleratorSignature__rl.GetBuffer(), instance->acceleratorSignature__rl.GetBufferSize(), 0, 0);
	Wait_sig__ReluLCacc(instance);// wait for everything to finish
}
inline void CreateBuffer_ReluLCacc_sig(int thread, InstanceData_sig__ReluLCacc* instance, float (*Pin), float (*Pout), int fsize, int chunk)
{
	int index, i;
	...
	instance->node_rl.SetSPMConfig((sizeof(float) * (chunk)) + (sizeof(float) * (chunk)), 3, 0);
	instance->LCAcc_FuncArgs__Pin = Pin;
	instance->LCAcc_FuncArgs__Pout = Pout;
	instance->LCAcc_FuncVars__fsize = fsize;
	instance->LCAcc_FuncVars__chunk = chunk;
	instance->allocatedAcceleratorCount__Relu = 0;
	LCAccNode& VNR_vardecl_0(instance->node_rl);
	void* VNR_vardecl_1(instance->LCAcc_FuncArgs__Pin);
	void* VNR_vardecl_2(instance->LCAcc_FuncArgs__Pout);
	...
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_5((0) + ((((0) * (sizeof(float))))), sizeof(float), VNR_vardecl_3, VNR_vardecl_4);
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_6(((sizeof(float) * (chunk))) + ((((0) * (sizeof(float))))), sizeof(float), VNR_vardecl_3, VNR_vardecl_4);
	...
	VNR_vardecl_9.push_back(((fsize) - (0)) / (chunk));
	...
	instance->acceleratorSignature__rl.AddCompute(VNR_vardecl_0, VNR_vardecl_7, VNR_vardecl_8);
	//produce transfer set
	//transfer from Pin to rl
	...
	instance->acceleratorSignature__rl.AddTransfer(VNR_vardecl_1, VNR_vardecl_9, VNR_vardecl_10, VNR_vardecl_3, VNR_vardecl_11, VNR_vardecl_0, (0) + ((((0) * ((sizeof(float)))))), VNR_vardecl_3, VNR_vardecl_11, sizeof(float));
	//transfer from rl to Pout
	...
	instance->acceleratorSignature__rl.AddTransfer(VNR_vardecl_0, ((sizeof(float) * (chunk))) + ((((0) * ((sizeof(float)))))), VNR_vardecl_3, VNR_vardecl_11, VNR_vardecl_2, VNR_vardecl_9, VNR_vardecl_10, VNR_vardecl_3, VNR_vardecl_11, sizeof(float));
	instance->acceleratorSignature__rl.Finalize((((fsize) - (0)) / (chunk)));
}
inline void ReluLCacc_sig_buf(int threadID, InstanceData_sig__ReluLCacc* instance)
{
	instance->Reset();
	LWI_RegisterInterruptHandler(&(instance->GAM_INTERACTION));
	LCAcc_Request(threadID, 246, 1);//request something of type AcceleratorType
	Wait_sig__ReluLCacc(instance);// wait for everything to finish
}
inline void ReluLCacc_sig(int threadID, float (*Pin), float (*Pout), int fsize, int chunk)
{
	InstanceData_sig__ReluLCacc instance;
	CreateBuffer_ReluLCacc_sig(threadID, &instance, Pin, Pout, fsize, chunk);
	ReluLCacc_sig_buf(threadID, &instance);
}
class BiN_ReluLCacc_Arbitrator_sig
{
	std::vector<InstanceData_sig__ReluLCacc*> instanceSet;
	...
	InterruptArgs isr;
public:
	inline BiN_ReluLCacc_Arbitrator_sig(int thread)
	{
		ops.push_back(246);
		count.push_back(1);
		threadID = thread;
	}
	inline void AddConfig(InstanceData_sig__ReluLCacc* inst, uint32_t performance, uint32_t cacheMod)
	{
		simics_assert(inst->acceleratorSignature__rl.IsFinalized());
	...
	inline void Run()
	{
		...
		int allocatedAcceleratorCount__Relu = 0;
		LCAcc_Request(threadID, 246, 1);//request something of type AcceleratorType
		bool cont = true;
				...
				case(LCACC_GAM_GRANT):
					{
						...
						{
							case(246):
								simics_assert(allocatedAcceleratorCount__Relu < 1);
								allocatedAcceleratorIDSet__Relu[allocatedAcceleratorCount__Relu] = lcaccID;
								allocatedAcceleratorCount__Relu++;
								break;
							default:
								simics_assert(0);
						}
						if(allocatedAcceleratorCount__Relu == 1)
						{
							cont = false;
						}
					}
		...
		InstanceData_sig__ReluLCacc* inst = NULL;
		for(size_t i = 0; i < instanceSet.size(); i++)
		{
		...
		for(int i = 0; i < 1; i++)
		{
			inst->allocatedAcceleratorIDSet__Relu[i] = allocatedAcceleratorIDSet__Relu[i];
		}
		inst->allocatedAcceleratorCount__Relu = allocatedAcceleratorCount__Relu;
		inst->pendingAccelerators = 1;
		StartEverythingHandler_sig__ReluLCacc(inst);
	}
};

#endif
#ifndef LCACC_BODY_TD__ReluLCacc__X
#define LCACC_BODY_TD__ReluLCacc__X

inline void Wait_td__ReluLCacc(int thread)
{
...
inline void CreateBuffer_ReluLCacc_td(uint8_t** buffer, uint32_t* bufferSize, uint8_t** constCluster, int thread, float (*Pin), float (*Pout), int fsize, int chunk)
{
	MicroprogramWriter mw(false);
	void* LCAcc_FuncArgs__Pin = Pin;
	void* LCAcc_FuncArgs__Pout = Pout;
	{
		void* VNR_vardecl_0(Pin);
		void* VNR_vardecl_1(Pout);
		LCAccNode VNR_vardecl_2(246, (sizeof(float) * (chunk)) + (sizeof(float) * (chunk)), 3, 0, 0);
		std::vector<uint32_t> VNR_vardecl_3;
		VNR_vardecl_3.push_back(((chunk) - (0)) / (1));
		std::vector<int32_t> VNR_vardecl_4;
		VNR_vardecl_4.push_back((1) * (sizeof(float)));
		...
		//Search VNR_vardecl_3 for destination size.
		//Search VNR_vardecl_11 for destination stride.
		mw.AddTransfer(VNR_vardecl_0, VNR_vardecl_9, VNR_vardecl_10, VNR_vardecl_3, VNR_vardecl_11, VNR_vardecl_2, (0) + ((((0) * ((sizeof(float)))))), VNR_vardecl_3, VNR_vardecl_11, sizeof(float));
		...
		//Search VNR_vardecl_3 for destination element size.
		//Search VNR_vardecl_11 for destination element stride.
		mw.AddTransfer(VNR_vardecl_2, ((sizeof(float) * (chunk))) + ((((0) * ((sizeof(float)))))), VNR_vardecl_3, VNR_vardecl_11, VNR_vardecl_1, VNR_vardecl_9, VNR_vardecl_10, VNR_vardecl_3, VNR_vardecl_11, sizeof(float));
		
		mw.SetTaskGrain(0);
		mw.Finalize((((fsize) - (0)) / (chunk)));
	}
...
inline void ReluLCacc_td_buf(uint8_t* buf, uint32_t bufSize, int thread)
{
	...
	Wait_td__ReluLCacc(thread);// wait for everything to finish
}
void ReluLCacc_td(int thread, float (*Pin), float (*Pout), int fsize, int chunk)
{
	uint32_t bufSize;
	uint8_t* buffer;
	uint8_t* constCluster = NULL;
	CreateBuffer_ReluLCacc_td(&buffer, &bufSize, &constCluster, thread, Pin, Pout, fsize, chunk);
	ReluLCacc_td_buf(buffer, bufSize, thread);
	if(constCluster)
	{
		delete [] constCluster;
	}
...
inline uint32_t ReluLCacc_CalculateBiNSize(int fsize, int chunk)
{
	return 0;
}
class BiN_ReluLCacc_Arbitrator_td
{
	...
	InterruptArgs isr;
public:
	inline BiN_ReluLCacc_Arbitrator_td(){}
	...
	inline void Run(int threadID)
	{
		...
		isr.lcaccID = 0;
		isr.lcaccMode = 0;
		LCAcc_DeclareLCAccUse(threadID, 246, 1); //requests for Relu
		...
		for(size_t i = 0; i < binSizeSet.size(); i++)
		{
			...
			{
				ReluLCacc_td_buf(bufSet[i], bufSizeSet[i], threadID);
				return;
			}
		}
		simics_assert(0);
