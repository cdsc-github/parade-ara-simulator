...
void TDLCA_Deblur::Initialize(int threadID, int procID)
{
        thread = threadID;
	...
	ret_u = new float[dataSize * dataSize * dataSize];
        ...
        CreateBuffer_Denoise1LCacc_td(&buf1, &buf1Size, &constCluster1, thread, u, g, dataSize, dataSize, dataSize, 8, 8, 8);
        CreateBuffer_Deblur1LCacc_td(&buf2, &buf2Size, &constCluster2, thread, conv, f, conv, 0.20f, dataSize, dataSize, dataSize, 1, 2, dataSize - 2);
        CreateBuffer_Deblur2LCacc_td(&buf3, &buf3Size, &constCluster3, thread,u, g, conv, ret_u, dataSize, dataSize, dataSize, 8, 8, 8, 1.0f, 1.0f);
        CreateBuffer_Blur1LCacc_td(&blur1_buf, &blur1_bufSize, &blur1_constCluster, thread, conv, conv, 1, dataSize, dataSize, dataSize, 0.20f, 1, 2, dataSize);
        CreateBuffer_Blur2LCacc_td(&blur2_buf, &blur2_bufSize, &blur2_constCluster, thread, conv, conv, 1, dataSize, dataSize, dataSize, 0.20f, 1, 2, dataSize);
        CreateBuffer_Blur3LCacc_td(&blur3_buf, &blur3_bufSize, &blur3_constCluster, thread, conv, conv, 1, dataSize, dataSize, dataSize, 0.20f, 1, 2, dataSize);
