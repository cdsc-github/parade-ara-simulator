...
void TDLCA_EKF_SLAM::Initialize(int threadID, int procID)
{
	...
	CreateBuffer_JacobiansLCacc_td(&buf1, &buf1Size, &constCluster1, thread, X, U, out_df_dx, out_X_oplus_U, dataSize, 128);
	CreateBuffer_SphericalCoordsLCacc_td(&buf2, &buf2Size, &constCluster2, thread, cpose, point, out_jacob_dryp_dpoint, out_jacob_dryp_dpose, dataSize, 128);
