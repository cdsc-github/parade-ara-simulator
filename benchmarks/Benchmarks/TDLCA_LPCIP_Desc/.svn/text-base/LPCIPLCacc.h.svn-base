...
#endif
#ifndef LCACC_BODY_SIG__LPCIPLCacc__X
#define LCACC_BODY_SIG__LPCIPLCacc__X
#define LCACC_CLASS_SIG__LPCIPLCacc__lpcip 1
class InstanceData_sig__LPCIPLCacc
{
public:
	...
	int allocatedAcceleratorCount__LPCIP;
	int allocatedAcceleratorIDSet__LPCIP[1];
	uint64_t (*LCAcc_FuncArgs__imgStart);
	float (*LCAcc_FuncArgs__x);
	float (*LCAcc_FuncArgs__y);
	float (*LCAcc_FuncArgs__rho);
	uint8_t (*LCAcc_FuncArgs__sample)[100];
	int LCAcc_FuncVars__totalSize;
	int LCAcc_FuncVars__chunk;
	MicroprogramWriter acceleratorSignature__lpcip;
	LCAccNode node_lpcip;
	InterruptArgs HandlerArgs__lpcip;
	inline void Reset()
	{
		pendingAccelerators = 0;
		allocatedAcceleratorCount__LPCIP = 0;
		reservedAccelerators = 0;
		allocatedAcceleratorIDSet__LPCIP[0] = 0;
		HandlerArgs__lpcip.threadID = threadID;
		HandlerArgs__lpcip.status = 0;
		HandlerArgs__lpcip.taskIndex = 0;
		HandlerArgs__lpcip.lcaccMode = LCACC_CLASS_SIG__LPCIPLCacc__lpcip;
		node_lpcip.Reset();
	}
...
inline void Cleanup_sig__LPCIPLCacc(InstanceData_sig__LPCIPLCacc* instance);
inline void StartEverythingHandler_sig__LPCIPLCacc(InstanceData_sig__LPCIPLCacc* instance);
inline void StopEverythingHandler_sig__LPCIPLCacc(InstanceData_sig__LPCIPLCacc* instance);
inline void ErrorHandler_sig__LPCIPLCacc(InstanceData_sig__LPCIPLCacc* instance);
inline void Wait_sig__LPCIPLCacc(InstanceData_sig__LPCIPLCacc* instance);
inline unsigned int DelayEstimate_sig__LPCIPLCacc(int vectorSize)
{
	return 1;
}
inline void (*ProgressHandler_sig__LPCIPLCacc__lpcip(InstanceData_sig__LPCIPLCacc* instance, InterruptArgs* args))(InstanceData_sig__LPCIPLCacc*);
//This is a procedurally generated interrupt-based program from a RAGraph data structure for function LPCIPLCacc
inline void Wait_sig__LPCIPLCacc(InstanceData_sig__LPCIPLCacc* instance)
{
	while(1)
	{
		...
		InterruptArgs* args = 0;
		void (*progress)(InstanceData_sig__LPCIPLCacc*);
		while((args = LWI_CheckInterrupt(thread)) == 0);
		switch(args->lcaccMode)
		{
			case(0):
				progress = GAMHandler_sig__LPCIPLCacc(instance, args);
				break;
			case(LCACC_CLASS_SIG__LPCIPLCacc__lpcip):
				progress = ProgressHandler_sig__LPCIPLCacc__lpcip(instance, args);
				break;
			default:
				ErrorHandler_sig__LPCIPLCacc(instance);
				simics_assert(0);
		}
	...
	switch(args->status)
	{
		case(LCACC_GAM_WAIT):
			...
			if(instance->reservedAccelerators == 1)
			{
				...
				LCAcc_Reserve(args->threadID, 0, DelayEstimate_sig__LPCIPLCacc(instance->acceleratorVectorLength));
			}
			return 0;
			break;
		case(LCACC_GAM_GRANT):
			...
			{
				case(806):
					if(instance->allocatedAcceleratorCount__LPCIP == 1)
					{
						return ErrorHandler_sig__LPCIPLCacc;
					}
					else
					{
						instance->pendingAccelerators++;
						instance->allocatedAcceleratorIDSet__LPCIP[instance->allocatedAcceleratorCount__LPCIP] = lcaccID;
						instance->allocatedAcceleratorCount__LPCIP++;
					}
					break;
				default:
					return ErrorHandler_sig__LPCIPLCacc;
			...
			if(instance->allocatedAcceleratorCount__LPCIP == 1)
			{
				return StartEverythingHandler_sig__LPCIPLCacc;
			}
			else
			{
				return 0;
...
inline void Cleanup_sig__LPCIPLCacc(InstanceData_sig__LPCIPLCacc* instance)
{
	...
	if(instance->allocatedAcceleratorIDSet__LPCIP[0] != 0)
	{
		LCAcc_Free(instance->HandlerArgs__lpcip.threadID, instance->HandlerArgs__lpcip.lcaccID);
		LWI_UnregisterInterruptHandler(instance->HandlerArgs__lpcip.threadID, instance->HandlerArgs__lpcip.lcaccID);
	}
}
inline void StopEverythingHandler_sig__LPCIPLCacc(InstanceData_sig__LPCIPLCacc* instance)
{
	Cleanup_sig__LPCIPLCacc(instance);
}
inline void ErrorHandler_sig__LPCIPLCacc(InstanceData_sig__LPCIPLCacc* instance)
{
	Cleanup_sig__LPCIPLCacc(instance);
...
inline void (*ProgressHandler_sig__LPCIPLCacc__lpcip(InstanceData_sig__LPCIPLCacc* instance, InterruptArgs* args))(InstanceData_sig__LPCIPLCacc*)
{
	switch(args->status)
	{
		...
		case(LCACC_STATUS_COMPLETED):
			...
			{
				return StopEverythingHandler_sig__LPCIPLCacc;
			}
			else
			{
				return 0;
...
{
	int index;
	instance->HandlerArgs__lpcip.threadID = instance->threadID;
	instance->HandlerArgs__lpcip.lcaccID = instance->allocatedAcceleratorIDSet__LPCIP[0];
	instance->node_lpcip.SetLCAccID(instance->HandlerArgs__lpcip.lcaccID);
	instance->HandlerArgs__lpcip.status = 0;
	instance->HandlerArgs__lpcip.taskIndex = 0;
	instance->HandlerArgs__lpcip.lcaccMode = LCACC_CLASS_SIG__LPCIPLCacc__lpcip;
	LWI_RegisterInterruptHandler(&(instance->HandlerArgs__lpcip));
	simics_assert(instance->acceleratorSignature__lpcip.IsFinalized());
	LCAcc_Command(instance->HandlerArgs__lpcip.threadID, instance->HandlerArgs__lpcip.lcaccID, LCACC_CMD_BEGIN_TASK_SIGNATURE, instance->acceleratorSignature__lpcip.GetBuffer(), instance->acceleratorSignature__lpcip.GetBufferSize(), 0, 0);
	Wait_sig__LPCIPLCacc(instance);// wait for everything to finish
}
inline void CreateBuffer_LPCIPLCacc_sig(int thread, InstanceData_sig__LPCIPLCacc* instance, uint64_t (*imgStart), float (*x), float (*y), float (*rho), uint8_t (*sample)[100], int totalSize, int chunk)
{
	...
	instance->node_lpcip.SetSPMConfig((sizeof(uint64_t) * (chunk)) + (sizeof(float) * (chunk)) + (sizeof(float) * (chunk)) + (sizeof(float) * (chunk)) + (sizeof(uint8_t[100]) * (chunk)), 3, 0);
	instance->LCAcc_FuncArgs__imgStart = imgStart;
	instance->LCAcc_FuncArgs__x = x;
	instance->LCAcc_FuncArgs__y = y;
	instance->LCAcc_FuncArgs__rho = rho;
	instance->LCAcc_FuncArgs__sample = sample;
	instance->LCAcc_FuncVars__totalSize = totalSize;
	instance->LCAcc_FuncVars__chunk = chunk;
	instance->allocatedAcceleratorCount__LPCIP = 0;
	LCAccNode& VNR_vardecl_0(instance->node_lpcip);
	void* VNR_vardecl_1(instance->LCAcc_FuncArgs__imgStart);
	void* VNR_vardecl_2(instance->LCAcc_FuncArgs__x);
	void* VNR_vardecl_3(instance->LCAcc_FuncArgs__y);
	void* VNR_vardecl_4(instance->LCAcc_FuncArgs__rho);
	void* VNR_vardecl_5(instance->LCAcc_FuncArgs__sample);
	std::vector<uint32_t> VNR_vardecl_6;
	VNR_vardecl_6.push_back(((chunk) - (0)) / (1));
	std::vector<int32_t> VNR_vardecl_7;
	VNR_vardecl_7.push_back((1) * (sizeof(uint64_t)));
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_8((0) + ((((0) * (sizeof(uint64_t))))), sizeof(uint64_t), VNR_vardecl_6, VNR_vardecl_7);
	std::vector<int32_t> VNR_vardecl_9;
	VNR_vardecl_9.push_back((1) * (sizeof(float)));
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_10(((sizeof(uint64_t) * (chunk))) + ((((0) * (sizeof(float))))), sizeof(float), VNR_vardecl_6, VNR_vardecl_9);
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_11(((sizeof(uint64_t) * (chunk)) + (sizeof(float) * (chunk))) + ((((0) * (sizeof(float))))), sizeof(float), VNR_vardecl_6, VNR_vardecl_9);
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_12(((sizeof(uint64_t) * (chunk)) + (sizeof(float) * (chunk)) + (sizeof(float) * (chunk))) + ((((0) * (sizeof(float))))), sizeof(float), VNR_vardecl_6, VNR_vardecl_9);
	...
	VNR_vardecl_14.push_back(ConvertToType<int, uint64_t>(480)/*Register LPCIP_IMGH*/);
	VNR_vardecl_14.push_back(ConvertToType<int, uint64_t>(640)/*Register LPCIP_IMGW*/);
	VNR_vardecl_14.push_back(ConvertToType<int, uint64_t>(10)/*Register LPCIP_PATCH_H*/);
	VNR_vardecl_14.push_back(ConvertToType<int, uint64_t>(10)/*Register LPCIP_PATCH_W*/);
	VNR_vardecl_14.push_back(ConvertToType<int, uint64_t>(((sizeof(uint64_t) * (chunk)) + (sizeof(float) * (chunk)) + (sizeof(float) * (chunk)) + (sizeof(float) * (chunk))))/*Register spmSampleTarget*/);
	std::vector<uint32_t> VNR_vardecl_15;
	VNR_vardecl_15.push_back(((totalSize) - (0)) / (chunk));
	std::vector<int32_t> VNR_vardecl_16;
	VNR_vardecl_16.push_back((chunk) * ((sizeof(uint64_t))));
	std::vector<int32_t> VNR_vardecl_17;
	VNR_vardecl_17.push_back((1) * ((sizeof(uint64_t))));
	std::vector<int32_t> VNR_vardecl_18;
	VNR_vardecl_18.push_back((chunk) * ((sizeof(float))));
	std::vector<int32_t> VNR_vardecl_19;
	VNR_vardecl_19.push_back((1) * ((sizeof(float))));
	std::vector<uint32_t> VNR_vardecl_20;
	VNR_vardecl_20.push_back(((chunk) - (0)) / (1));
	VNR_vardecl_20.push_back((100));
	std::vector<int32_t> VNR_vardecl_21;
	VNR_vardecl_21.push_back((1) * ((sizeof(uint8_t[100]))));
	VNR_vardecl_21.push_back((sizeof(uint8_t)));
	std::vector<int32_t> VNR_vardecl_22;
	VNR_vardecl_22.push_back((chunk) * ((sizeof(uint8_t[100]))));
	//produce compute set
	//See VNR_vardecl_13 for index variable decl
	//See VNR_vardecl_14 for register set decl
	instance->acceleratorSignature__lpcip.AddCompute(VNR_vardecl_0, VNR_vardecl_13, VNR_vardecl_14);
	...
	//Search VNR_vardecl_6 for source element size.
	...
	instance->acceleratorSignature__lpcip.AddTransfer(VNR_vardecl_1, VNR_vardecl_15, VNR_vardecl_16, VNR_vardecl_6, VNR_vardecl_17, VNR_vardecl_0, (0) + ((((0) * ((sizeof(uint64_t)))))), VNR_vardecl_6, VNR_vardecl_17, sizeof(uint64_t));
	...
	instance->acceleratorSignature__lpcip.AddTransfer(VNR_vardecl_2, VNR_vardecl_15, VNR_vardecl_18, VNR_vardecl_6, VNR_vardecl_19, VNR_vardecl_0, ((sizeof(uint64_t) * (chunk))) + ((((0) * ((sizeof(float)))))), VNR_vardecl_6, VNR_vardecl_19, sizeof(float));
	...
	//Search VNR_vardecl_6 for destination size.
	//Search VNR_vardecl_19 for destination stride.
	instance->acceleratorSignature__lpcip.AddTransfer(VNR_vardecl_3, VNR_vardecl_15, VNR_vardecl_18, VNR_vardecl_6, VNR_vardecl_19, VNR_vardecl_0, ((sizeof(uint64_t) * (chunk)) + (sizeof(float) * (chunk))) + ((((0) * ((sizeof(float)))))), VNR_vardecl_6, VNR_vardecl_19, sizeof(float));
	...
	//Search VNR_vardecl_6 for destination size.
	//Search VNR_vardecl_19 for destination stride.
	instance->acceleratorSignature__lpcip.AddTransfer(VNR_vardecl_4, VNR_vardecl_15, VNR_vardecl_18, VNR_vardecl_6, VNR_vardecl_19, VNR_vardecl_0, ((sizeof(uint64_t) * (chunk)) + (sizeof(float) * (chunk)) + (sizeof(float) * (chunk))) + ((((0) * ((sizeof(float)))))), VNR_vardecl_6, VNR_vardecl_19, sizeof(float));
	...
	//Search VNR_vardecl_22 for destination block stride.
	//Search VNR_vardecl_20 for destination element size.
	//Search VNR_vardecl_21 for destination element stride.
	instance->acceleratorSignature__lpcip.AddTransfer(VNR_vardecl_0, ((sizeof(uint64_t) * (chunk)) + (sizeof(float) * (chunk)) + (sizeof(float) * (chunk)) + (sizeof(float) * (chunk))) + ((((0) * ((sizeof(uint8_t[100])))))), VNR_vardecl_20, VNR_vardecl_21, VNR_vardecl_5, VNR_vardecl_15, VNR_vardecl_22, VNR_vardecl_20, VNR_vardecl_21, sizeof(uint8_t));
	instance->acceleratorSignature__lpcip.Finalize((((totalSize) - (0)) / (chunk)));
}
inline void LPCIPLCacc_sig_buf(int threadID, InstanceData_sig__LPCIPLCacc* instance)
{
	instance->Reset();
	LWI_RegisterInterruptHandler(&(instance->GAM_INTERACTION));
	LCAcc_Request(threadID, 806, 1);//request something of type AcceleratorType
	Wait_sig__LPCIPLCacc(instance);// wait for everything to finish
}
inline void LPCIPLCacc_sig(int threadID, uint64_t (*imgStart), float (*x), float (*y), float (*rho), uint8_t (*sample)[100], int totalSize, int chunk)
{
	InstanceData_sig__LPCIPLCacc instance;
	CreateBuffer_LPCIPLCacc_sig(threadID, &instance, imgStart, x, y, rho, sample, totalSize, chunk);
	LPCIPLCacc_sig_buf(threadID, &instance);
}
class BiN_LPCIPLCacc_Arbitrator_sig
{
	std::vector<InstanceData_sig__LPCIPLCacc*> instanceSet;
	...
	InterruptArgs isr;
public:
	inline BiN_LPCIPLCacc_Arbitrator_sig(int thread)
	{
		ops.push_back(806);
		count.push_back(1);
		threadID = thread;
	}
	inline void AddConfig(InstanceData_sig__LPCIPLCacc* inst, uint32_t performance, uint32_t cacheMod)
	{
		simics_assert(inst->acceleratorSignature__lpcip.IsFinalized());
	...
	inline void Run()
	{
		...
		int allocatedAcceleratorCount__LPCIP = 0;
		LCAcc_Request(threadID, 806, 1);//request something of type AcceleratorType
		bool cont = true;
				...
				case(LCACC_GAM_GRANT):
					{
						...
						{
							case(806):
								simics_assert(allocatedAcceleratorCount__LPCIP < 1);
								allocatedAcceleratorIDSet__LPCIP[allocatedAcceleratorCount__LPCIP] = lcaccID;
								allocatedAcceleratorCount__LPCIP++;
								break;
							default:
								simics_assert(0);
						}
						if(allocatedAcceleratorCount__LPCIP == 1)
						{
							cont = false;
						}
					}
		...
		InstanceData_sig__LPCIPLCacc* inst = NULL;
		for(size_t i = 0; i < instanceSet.size(); i++)
		{
		...
		for(int i = 0; i < 1; i++)
		{
			inst->allocatedAcceleratorIDSet__LPCIP[i] = allocatedAcceleratorIDSet__LPCIP[i];
		}
		inst->allocatedAcceleratorCount__LPCIP = allocatedAcceleratorCount__LPCIP;
		inst->pendingAccelerators = 1;
		StartEverythingHandler_sig__LPCIPLCacc(inst);
	}
};

#endif
#ifndef LCACC_BODY_TD__LPCIPLCacc__X
#define LCACC_BODY_TD__LPCIPLCacc__X

inline void Wait_td__LPCIPLCacc(int thread)
{
...
inline void CreateBuffer_LPCIPLCacc_td(uint8_t** buffer, uint32_t* bufferSize, uint8_t** constCluster, int thread, uint64_t (*imgStart), float (*x), float (*y), float (*rho), uint8_t (*sample)[100], int totalSize, int chunk)
{
	simics_assert(chunk > 0);
	simics_assert((totalSize) > 0);
	simics_assert((totalSize) % chunk == 0);
	MicroprogramWriter mw(false);
	void* LCAcc_FuncArgs__imgStart = imgStart;
	void* LCAcc_FuncArgs__x = x;
	void* LCAcc_FuncArgs__y = y;
	void* LCAcc_FuncArgs__rho = rho;
	void* LCAcc_FuncArgs__sample = sample;
	{
		void* VNR_vardecl_0(imgStart);
		void* VNR_vardecl_1(x);
...
inline void LPCIPLCacc_td_buf(uint8_t* buf, uint32_t bufSize, int thread)
{
	...
	Wait_td__LPCIPLCacc(thread);// wait for everything to finish
}
void LPCIPLCacc_td(int thread, uint64_t (*imgStart), float (*x), float (*y), float (*rho), uint8_t (*sample)[100], int totalSize, int chunk)
{
	uint32_t bufSize;
	uint8_t* buffer;
	uint8_t* constCluster = NULL;
	CreateBuffer_LPCIPLCacc_td(&buffer, &bufSize, &constCluster, thread, imgStart, x, y, rho, sample, totalSize, chunk);
	LPCIPLCacc_td_buf(buffer, bufSize, thread);
	if(constCluster)
	{
		delete [] constCluster;
	}
...
inline uint32_t LPCIPLCacc_CalculateBiNSize(int totalSize, int chunk)
{
	return 0;
}
class BiN_LPCIPLCacc_Arbitrator_td
{
	...
	InterruptArgs isr;
public:
	inline BiN_LPCIPLCacc_Arbitrator_td(){}
	...
	inline void Run(int threadID)
	{
		...
		isr.lcaccID = 0;
		isr.lcaccMode = 0;
		LCAcc_DeclareLCAccUse(threadID, 806, 1); //requests for LPCIP
		...
		for(size_t i = 0; i < binSizeSet.size(); i++)
		{
			...
			{
				LPCIPLCacc_td_buf(bufSet[i], bufSizeSet[i], threadID);
				return;
			}
		}
		simics_assert(0);
