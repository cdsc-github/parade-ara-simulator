...
#endif
#ifndef LCACC_BODY_SIG__Blur2LCacc__X
#define LCACC_BODY_SIG__Blur2LCacc__X
#define LCACC_CLASS_SIG__Blur2LCacc__b 1
class InstanceData_sig__Blur2LCacc
{
public:
	...
	inline void Reset()
	{
		...
		HandlerArgs__b.lcaccMode = LCACC_CLASS_SIG__Blur2LCacc__b;
		node_b.Reset();
	}
	inline InstanceData_sig__Blur2LCacc() :
...
inline void (*GAMHandler_sig__Blur2LCacc(InstanceData_sig__Blur2LCacc* instance, InterruptArgs* args))(InstanceData_sig__Blur2LCacc*) ;
inline void Cleanup_sig__Blur2LCacc(InstanceData_sig__Blur2LCacc* instance);
inline void StartEverythingHandler_sig__Blur2LCacc(InstanceData_sig__Blur2LCacc* instance);
inline void StopEverythingHandler_sig__Blur2LCacc(InstanceData_sig__Blur2LCacc* instance);
inline void ErrorHandler_sig__Blur2LCacc(InstanceData_sig__Blur2LCacc* instance);
inline void Wait_sig__Blur2LCacc(InstanceData_sig__Blur2LCacc* instance);
inline unsigned int DelayEstimate_sig__Blur2LCacc(int vectorSize)
{
	return 1;
}
inline void (*ProgressHandler_sig__Blur2LCacc__b(InstanceData_sig__Blur2LCacc* instance, InterruptArgs* args))(InstanceData_sig__Blur2LCacc*);
//This is a procedurally generated interrupt-based program from a RAGraph data structure for function Blur2LCacc
inline void Wait_sig__Blur2LCacc(InstanceData_sig__Blur2LCacc* instance)
{
	while(1)
	{
		...
		InterruptArgs* args = 0;
		void (*progress)(InstanceData_sig__Blur2LCacc*);
		while((args = LWI_CheckInterrupt(thread)) == 0);
		switch(args->lcaccMode)
		{
			case(0):
				progress = GAMHandler_sig__Blur2LCacc(instance, args);
				break;
			case(LCACC_CLASS_SIG__Blur2LCacc__b):
				progress = ProgressHandler_sig__Blur2LCacc__b(instance, args);
				break;
			default:
				ErrorHandler_sig__Blur2LCacc(instance);
				simics_assert(0);
		}
	...
	switch(args->status)
	{
		case(LCACC_GAM_WAIT):
			...
			if(instance->reservedAccelerators == 1)
			{
				...
				LCAcc_Reserve(args->threadID, 0, DelayEstimate_sig__Blur2LCacc(instance->acceleratorVectorLength));
			}
			return 0;
			break;
		case(LCACC_GAM_GRANT):
			...
			switch(lcaccMode)
			{
				case(105):
					...
					{
						return ErrorHandler_sig__Blur2LCacc;
					}
					else
					{
			...
			if(instance->allocatedAcceleratorCount__blur1Mega == 1)
			{
				return StartEverythingHandler_sig__Blur2LCacc;
			}
			else
			{
				return 0;
...
inline void StopEverythingHandler_sig__Blur2LCacc(InstanceData_sig__Blur2LCacc* instance)
{
	Cleanup_sig__Blur2LCacc(instance);
}
inline void ErrorHandler_sig__Blur2LCacc(InstanceData_sig__Blur2LCacc* instance)
{
	Cleanup_sig__Blur2LCacc(instance);
...
inline void (*ProgressHandler_sig__Blur2LCacc__b(InstanceData_sig__Blur2LCacc* instance, InterruptArgs* args))(InstanceData_sig__Blur2LCacc*)
{
	switch(args->status)
	{
		...
		case(LCACC_STATUS_COMPLETED):
			...
			{
				return StopEverythingHandler_sig__Blur2LCacc;
			}
			else
			{
				return 0;
...
inline void StartEverythingHandler_sig__Blur2LCacc(InstanceData_sig__Blur2LCacc* instance)
{
	...
	instance->HandlerArgs__b.lcaccMode = LCACC_CLASS_SIG__Blur2LCacc__b;
	...
	Wait_sig__Blur2LCacc(instance);// wait for everything to finish
}
inline void CreateBuffer_Blur2LCacc_sig(int thread, InstanceData_sig__Blur2LCacc* instance, float (*in), float (*out), int channelCount, int sizeX, int sizeY, int sizeZ, float nu, int chunkX, int chunkY, int chunkZ)
{
	...
	VNR_vardecl_11.push_back(((chunkZ - 1)) * ((sizeof(float) * sizeY)));
	...
	VNR_vardecl_13.push_back((1) * ((sizeof(float) * sizeY)));
...
inline void Blur2LCacc_sig_buf(int threadID, InstanceData_sig__Blur2LCacc* instance)
{
	instance->Reset();
...
inline void Blur2LCacc_sig(int threadID, float (*in), float (*out), int channelCount, int sizeX, int sizeY, int sizeZ, float nu, int chunkX, int chunkY, int chunkZ)
{
	InstanceData_sig__Blur2LCacc instance;
	CreateBuffer_Blur2LCacc_sig(threadID, &instance, in, out, channelCount, sizeX, sizeY, sizeZ, nu, chunkX, chunkY, chunkZ);
	Blur2LCacc_sig_buf(threadID, &instance);
}
class BiN_Blur2LCacc_Arbitrator_sig
{
	std::vector<InstanceData_sig__Blur2LCacc*> instanceSet;
	...
	InterruptArgs isr;
public:
	inline BiN_Blur2LCacc_Arbitrator_sig(int thread)
	{
		ops.push_back(105);
		count.push_back(1);
		threadID = thread;
	}
	inline void AddConfig(InstanceData_sig__Blur2LCacc* inst, uint32_t performance, uint32_t cacheMod)
	{
	...
	inline void Run()
	{
		...
		InstanceData_sig__Blur2LCacc* inst = NULL;
		for(size_t i = 0; i < instanceSet.size(); i++)
		{
		...
		StartEverythingHandler_sig__Blur2LCacc(inst);
	}
};

#endif
#ifndef LCACC_BODY_TD__Blur2LCacc__X
#define LCACC_BODY_TD__Blur2LCacc__X

inline void Wait_td__Blur2LCacc(int thread)
{
...
inline void CreateBuffer_Blur2LCacc_td(uint8_t** buffer, uint32_t* bufferSize, uint8_t** constCluster, int thread, float (*in), float (*out), int channelCount, int sizeX, int sizeY, int sizeZ, float nu, int chunkX, int chunkY, int chunkZ)
{
	simics_assert(chunkX > 0);
	simics_assert(chunkY > 0);
	simics_assert(chunkZ > 0);
...
inline void Blur2LCacc_td_buf(uint8_t* buf, uint32_t bufSize, int thread)
{
	...
	Wait_td__Blur2LCacc(thread);// wait for everything to finish
}
void Blur2LCacc_td(int thread, float (*in), float (*out), int channelCount, int sizeX, int sizeY, int sizeZ, float nu, int chunkX, int chunkY, int chunkZ)
{
	uint32_t bufSize;
	uint8_t* buffer;
	uint8_t* constCluster = NULL;
	CreateBuffer_Blur2LCacc_td(&buffer, &bufSize, &constCluster, thread, in, out, channelCount, sizeX, sizeY, sizeZ, nu, chunkX, chunkY, chunkZ);
	Blur2LCacc_td_buf(buffer, bufSize, thread);
	if(constCluster)
	{
		delete [] constCluster;
	}
...
inline uint32_t Blur2LCacc_CalculateBiNSize(int channelCount, int sizeX, int sizeY, int sizeZ, float nu, int chunkX, int chunkY, int chunkZ)
{
	return 0;
}
class BiN_Blur2LCacc_Arbitrator_td
{
	...
	InterruptArgs isr;
public:
	inline BiN_Blur2LCacc_Arbitrator_td(){}
	...
	inline void Run(int threadID)
	{
		...
		for(size_t i = 0; i < binSizeSet.size(); i++)
		{
			...
			{
				Blur2LCacc_td_buf(bufSet[i], bufSizeSet[i], threadID);
				return;
			}
		}
		simics_assert(0);
