...
#endif
#ifndef LCACC_BODY_SIG__Segmentation1LCacc__X
#define LCACC_BODY_SIG__Segmentation1LCacc__X
#define LCACC_CLASS_SIG__Segmentation1LCacc__s 1
class InstanceData_sig__Segmentation1LCacc
{
public:
	...
	int allocatedAcceleratorCount__segmentation1Mega;
	int allocatedAcceleratorIDSet__segmentation1Mega[1];
	float (*LCAcc_FuncArgs__phi);
	float (*LCAcc_FuncArgs__u0);
	float (*LCAcc_FuncArgs__result);
	...
	float LCAcc_FuncVars__dx;
	float LCAcc_FuncVars__dy;
	float LCAcc_FuncVars__dz;
	float LCAcc_FuncVars__lambda1;
	float LCAcc_FuncVars__lambda2;
	float LCAcc_FuncVars__mu;
	float LCAcc_FuncVars__c1;
	float LCAcc_FuncVars__c2;
	...
	inline void Reset()
	{
		...
		allocatedAcceleratorCount__segmentation1Mega = 0;
		reservedAccelerators = 0;
		allocatedAcceleratorIDSet__segmentation1Mega[0] = 0;
		HandlerArgs__s.threadID = threadID;
		HandlerArgs__s.status = 0;
		HandlerArgs__s.taskIndex = 0;
		HandlerArgs__s.lcaccMode = LCACC_CLASS_SIG__Segmentation1LCacc__s;
		node_s.Reset();
	}
	inline InstanceData_sig__Segmentation1LCacc() :
...
inline void (*GAMHandler_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance, InterruptArgs* args))(InstanceData_sig__Segmentation1LCacc*) ;
inline void Cleanup_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance);
inline void StartEverythingHandler_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance);
inline void StopEverythingHandler_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance);
inline void ErrorHandler_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance);
inline void Wait_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance);
inline unsigned int DelayEstimate_sig__Segmentation1LCacc(int vectorSize)
{
	return 1;
}
inline void (*ProgressHandler_sig__Segmentation1LCacc__s(InstanceData_sig__Segmentation1LCacc* instance, InterruptArgs* args))(InstanceData_sig__Segmentation1LCacc*);
//This is a procedurally generated interrupt-based program from a RAGraph data structure for function Segmentation1LCacc
inline void Wait_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance)
{
	while(1)
	{
		...
		InterruptArgs* args = 0;
		void (*progress)(InstanceData_sig__Segmentation1LCacc*);
		while((args = LWI_CheckInterrupt(thread)) == 0);
		switch(args->lcaccMode)
		{
			case(0):
				progress = GAMHandler_sig__Segmentation1LCacc(instance, args);
				break;
			case(LCACC_CLASS_SIG__Segmentation1LCacc__s):
				progress = ProgressHandler_sig__Segmentation1LCacc__s(instance, args);
				break;
			default:
				ErrorHandler_sig__Segmentation1LCacc(instance);
				simics_assert(0);
		}
...
inline void (*GAMHandler_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance, InterruptArgs* args))(InstanceData_sig__Segmentation1LCacc*)
{
	...
	switch(args->status)
	{
		case(LCACC_GAM_WAIT):
			...
			switch(lcaccMode)
			{
				case(301)://Mode: segmentation1Mega
					for(i = 0; i < 1; i++)
					{
			...
			if(instance->reservedAccelerators == 1)
			{
				...
				LCAcc_Reserve(args->threadID, 0, DelayEstimate_sig__Segmentation1LCacc(instance->acceleratorVectorLength));
			}
			return 0;
			break;
		case(LCACC_GAM_GRANT):
			...
			{
				case(301):
					if(instance->allocatedAcceleratorCount__segmentation1Mega == 1)
					{
						return ErrorHandler_sig__Segmentation1LCacc;
					}
					else
					{
						instance->pendingAccelerators++;
						instance->allocatedAcceleratorIDSet__segmentation1Mega[instance->allocatedAcceleratorCount__segmentation1Mega] = lcaccID;
						instance->allocatedAcceleratorCount__segmentation1Mega++;
					}
					break;
				default:
					return ErrorHandler_sig__Segmentation1LCacc;
			...
			if(instance->allocatedAcceleratorCount__segmentation1Mega == 1)
			{
				return StartEverythingHandler_sig__Segmentation1LCacc;
			}
			else
			{
				return 0;
...
inline void Cleanup_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance)
{
	...
	if(instance->allocatedAcceleratorIDSet__segmentation1Mega[0] != 0)
	{
...
inline void StopEverythingHandler_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance)
{
	Cleanup_sig__Segmentation1LCacc(instance);
}
inline void ErrorHandler_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance)
{
	Cleanup_sig__Segmentation1LCacc(instance);
...
inline void (*ProgressHandler_sig__Segmentation1LCacc__s(InstanceData_sig__Segmentation1LCacc* instance, InterruptArgs* args))(InstanceData_sig__Segmentation1LCacc*)
{
	switch(args->status)
	{
		...
		case(LCACC_STATUS_COMPLETED):
			...
			{
				return StopEverythingHandler_sig__Segmentation1LCacc;
			}
			else
			{
				return 0;
...
inline void StartEverythingHandler_sig__Segmentation1LCacc(InstanceData_sig__Segmentation1LCacc* instance)
{
	...
	instance->HandlerArgs__s.lcaccID = instance->allocatedAcceleratorIDSet__segmentation1Mega[0];
	...
	instance->HandlerArgs__s.lcaccMode = LCACC_CLASS_SIG__Segmentation1LCacc__s;
	...
	Wait_sig__Segmentation1LCacc(instance);// wait for everything to finish
}
inline void CreateBuffer_Segmentation1LCacc_sig(int thread, InstanceData_sig__Segmentation1LCacc* instance, float (*phi), float (*u0), float (*result), int sizeX, int sizeY, int sizeZ, int chunkX, int chunkY, int chunkZ, float dx, float dy, float dz, float lambda1, float lambda2, float mu, float c1, float c2)
{
	...
	simics_assert(phi != result);
	simics_assert(u0 != result);
	int index, i;
	...
	instance->node_s.SetSPMConfig((sizeof(float) * (chunkX + 2) * (chunkY + 2) * (chunkZ + 2)) + (sizeof(float) * (chunkX) * (chunkY) * (chunkZ)) + (sizeof(float) * (chunkX) * (chunkY) * (chunkZ)), 3, 0);
	instance->LCAcc_FuncArgs__phi = phi;
	instance->LCAcc_FuncArgs__u0 = u0;
	instance->LCAcc_FuncArgs__result = result;
	...
	instance->LCAcc_FuncVars__dx = dx;
	instance->LCAcc_FuncVars__dy = dy;
	instance->LCAcc_FuncVars__dz = dz;
	instance->LCAcc_FuncVars__lambda1 = lambda1;
	instance->LCAcc_FuncVars__lambda2 = lambda2;
	instance->LCAcc_FuncVars__mu = mu;
	instance->LCAcc_FuncVars__c1 = c1;
	instance->LCAcc_FuncVars__c2 = c2;
	instance->allocatedAcceleratorCount__segmentation1Mega = 0;
	LCAccNode& VNR_vardecl_0(instance->node_s);
	void* VNR_vardecl_1(instance->LCAcc_FuncArgs__phi);
	void* VNR_vardecl_2(instance->LCAcc_FuncArgs__u0);
	void* VNR_vardecl_3(instance->LCAcc_FuncArgs__result);
	std::vector<uint32_t> VNR_vardecl_4;
	VNR_vardecl_4.push_back(((chunkX + 1) - (1)) / (1));
	VNR_vardecl_4.push_back(((chunkY + 1) - (1)) / (1));
	VNR_vardecl_4.push_back(((chunkZ + 1) - (1)) / (1));
	std::vector<int32_t> VNR_vardecl_5;
	VNR_vardecl_5.push_back((1) * (sizeof(float) * (chunkY + 2) * (chunkZ + 2)));
	VNR_vardecl_5.push_back((1) * (sizeof(float) * (chunkZ + 2)));
	VNR_vardecl_5.push_back((1) * (sizeof(float)));
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_6((0) + ((((1) * (sizeof(float) * (chunkY + 2) * (chunkZ + 2))) + ((1) * (sizeof(float) * (chunkZ + 2))) + ((1) * (sizeof(float))))), sizeof(float), VNR_vardecl_4, VNR_vardecl_5);
	std::vector<uint32_t> VNR_vardecl_7;
	VNR_vardecl_7.push_back(((chunkX) - (0)) / (1));
	VNR_vardecl_7.push_back(((chunkY) - (0)) / (1));
	VNR_vardecl_7.push_back(((chunkZ) - (0)) / (1));
	std::vector<int32_t> VNR_vardecl_8;
	VNR_vardecl_8.push_back((1) * (sizeof(float) * (chunkY) * (chunkZ)));
	VNR_vardecl_8.push_back((1) * (sizeof(float) * (chunkZ)));
	VNR_vardecl_8.push_back((1) * (sizeof(float)));
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_9(((sizeof(float) * (chunkX + 2) * (chunkY + 2) * (chunkZ + 2))) + ((((0) * (sizeof(float) * (chunkY) * (chunkZ))) + ((0) * (sizeof(float) * (chunkZ))) + ((0) * (sizeof(float))))), sizeof(float), VNR_vardecl_7, VNR_vardecl_8);
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_10(((sizeof(float) * (chunkX + 2) * (chunkY + 2) * (chunkZ + 2)) + (sizeof(float) * (chunkX) * (chunkY) * (chunkZ))) + ((((0) * (sizeof(float) * (chunkY) * (chunkZ))) + ((0) * (sizeof(float) * (chunkZ))) + ((0) * (sizeof(float))))), sizeof(float), VNR_vardecl_7, VNR_vardecl_8);
	std::vector<MicroprogramWriter::ComputeArgIndex> VNR_vardecl_11;
	VNR_vardecl_11.push_back(VNR_vardecl_6);
	VNR_vardecl_11.push_back(VNR_vardecl_9);
	VNR_vardecl_11.push_back(VNR_vardecl_10);
	std::vector<uint64_t> VNR_vardecl_12;
	VNR_vardecl_12.push_back(ConvertToType<int, uint64_t>(chunkX)/*Register width*/);
	VNR_vardecl_12.push_back(ConvertToType<int, uint64_t>(chunkY)/*Register height*/);
	VNR_vardecl_12.push_back(ConvertToType<int, uint64_t>(chunkZ)/*Register depth*/);
	VNR_vardecl_12.push_back(ConvertToType<float, uint64_t>(dx)/*Register dx*/);
	VNR_vardecl_12.push_back(ConvertToType<float, uint64_t>(dy)/*Register dy*/);
	VNR_vardecl_12.push_back(ConvertToType<float, uint64_t>(dz)/*Register dz*/);
	VNR_vardecl_12.push_back(ConvertToType<float, uint64_t>(lambda1)/*Register lambda1*/);
	VNR_vardecl_12.push_back(ConvertToType<float, uint64_t>(lambda2)/*Register lambda2*/);
	VNR_vardecl_12.push_back(ConvertToType<float, uint64_t>(mu)/*Register mu*/);
	VNR_vardecl_12.push_back(ConvertToType<float, uint64_t>(c1)/*Register c1*/);
	VNR_vardecl_12.push_back(ConvertToType<float, uint64_t>(c2)/*Register c2*/);
	std::vector<uint32_t> VNR_vardecl_13;
	VNR_vardecl_13.push_back(((sizeX - 1) - (1)) / (chunkX));
	VNR_vardecl_13.push_back(((sizeY - 1) - (1)) / (chunkY));
	VNR_vardecl_13.push_back(((sizeZ - 1) - (1)) / (chunkZ));
	std::vector<int32_t> VNR_vardecl_14;
	VNR_vardecl_14.push_back((chunkX) * ((sizeof(float) * (sizeY) * (sizeZ))));
	VNR_vardecl_14.push_back((chunkY) * ((sizeof(float) * (sizeZ))));
	VNR_vardecl_14.push_back((chunkZ) * ((sizeof(float))));
	std::vector<uint32_t> VNR_vardecl_15;
	VNR_vardecl_15.push_back(((chunkX + 2) - (0)) / (1));
	VNR_vardecl_15.push_back(((chunkY + 2) - (0)) / (1));
	VNR_vardecl_15.push_back(((chunkZ + 2) - (0)) / (1));
	std::vector<int32_t> VNR_vardecl_16;
	VNR_vardecl_16.push_back((1) * ((sizeof(float) * (sizeY) * (sizeZ))));
	VNR_vardecl_16.push_back((1) * ((sizeof(float) * (sizeZ))));
	VNR_vardecl_16.push_back((1) * ((sizeof(float))));
	std::vector<int32_t> VNR_vardecl_17;
	VNR_vardecl_17.push_back((1) * ((sizeof(float)) * (chunkY + 2) * (chunkZ + 2)));
	VNR_vardecl_17.push_back((1) * ((sizeof(float)) * (chunkZ + 2)));
	VNR_vardecl_17.push_back((1) * ((sizeof(float))));
	std::vector<uint32_t> VNR_vardecl_18;
	VNR_vardecl_18.push_back(((chunkY) - (0)) / (1));
	VNR_vardecl_18.push_back(((chunkY) - (0)) / (1));
	VNR_vardecl_18.push_back(((chunkZ) - (0)) / (1));
	std::vector<int32_t> VNR_vardecl_19;
	VNR_vardecl_19.push_back((1) * ((sizeof(float)) * (chunkY) * (chunkZ)));
	VNR_vardecl_19.push_back((1) * ((sizeof(float)) * (chunkZ)));
	...
	instance->acceleratorSignature__s.AddCompute(VNR_vardecl_0, VNR_vardecl_11, VNR_vardecl_12);
	//produce transfer set
	//transfer from phi to s
	//Search VNR_vardecl_13 for source block size.
	//Search VNR_vardecl_14 for source block stride.
	//Search VNR_vardecl_15 for source element size.
	//Search VNR_vardecl_16 for source element stride.
	//Search VNR_vardecl_15 for destination size.
	//Search VNR_vardecl_17 for destination stride.
	instance->acceleratorSignature__s.AddTransfer(VNR_vardecl_1, VNR_vardecl_13, VNR_vardecl_14, VNR_vardecl_15, VNR_vardecl_16, VNR_vardecl_0, (0) + ((((0) * ((sizeof(float)) * (chunkY + 2) * (chunkZ + 2))) + ((0) * ((sizeof(float)) * (chunkZ + 2))) + ((0) * ((sizeof(float)))))), VNR_vardecl_15, VNR_vardecl_17, sizeof(float));
	//transfer from u0 to s
	//Search VNR_vardecl_13 for source block size.
	//Search VNR_vardecl_14 for source block stride.
	...
	instance->acceleratorSignature__s.AddTransfer(VNR_vardecl_2, VNR_vardecl_13, VNR_vardecl_14, VNR_vardecl_18, VNR_vardecl_16, VNR_vardecl_0, ((sizeof(float) * (chunkX + 2) * (chunkY + 2) * (chunkZ + 2))) + ((((0) * ((sizeof(float)) * (chunkY) * (chunkZ))) + ((0) * ((sizeof(float)) * (chunkZ))) + ((0) * ((sizeof(float)))))), VNR_vardecl_18, VNR_vardecl_19, sizeof(float));
	//transfer from s to result
	//Search VNR_vardecl_7 for source size.
	...
	//Search VNR_vardecl_7 for destination element size.
	//Search VNR_vardecl_16 for destination element stride.
	instance->acceleratorSignature__s.AddTransfer(VNR_vardecl_0, ((sizeof(float) * (chunkX + 2) * (chunkY + 2) * (chunkZ + 2)) + (sizeof(float) * (chunkX) * (chunkY) * (chunkZ))) + ((((0) * ((sizeof(float)) * (chunkY) * (chunkZ))) + ((0) * ((sizeof(float)) * (chunkZ))) + ((0) * ((sizeof(float)))))), VNR_vardecl_7, VNR_vardecl_19, VNR_vardecl_3, VNR_vardecl_13, VNR_vardecl_14, VNR_vardecl_7, VNR_vardecl_16, sizeof(float));
	instance->acceleratorSignature__s.Finalize((((sizeX - 1) - (1)) / (chunkX)) * (((sizeY - 1) - (1)) / (chunkY)) * (((sizeZ - 1) - (1)) / (chunkZ)));
}
inline void Segmentation1LCacc_sig_buf(int threadID, InstanceData_sig__Segmentation1LCacc* instance)
{
	instance->Reset();
	LWI_RegisterInterruptHandler(&(instance->GAM_INTERACTION));
	LCAcc_Request(threadID, 301, 1);//request something of type AcceleratorType
	Wait_sig__Segmentation1LCacc(instance);// wait for everything to finish
}
inline void Segmentation1LCacc_sig(int threadID, float (*phi), float (*u0), float (*result), int sizeX, int sizeY, int sizeZ, int chunkX, int chunkY, int chunkZ, float dx, float dy, float dz, float lambda1, float lambda2, float mu, float c1, float c2)
{
	InstanceData_sig__Segmentation1LCacc instance;
	CreateBuffer_Segmentation1LCacc_sig(threadID, &instance, phi, u0, result, sizeX, sizeY, sizeZ, chunkX, chunkY, chunkZ, dx, dy, dz, lambda1, lambda2, mu, c1, c2);
	Segmentation1LCacc_sig_buf(threadID, &instance);
}
class BiN_Segmentation1LCacc_Arbitrator_sig
{
	std::vector<InstanceData_sig__Segmentation1LCacc*> instanceSet;
	...
	InterruptArgs isr;
public:
	inline BiN_Segmentation1LCacc_Arbitrator_sig(int thread)
	{
		ops.push_back(301);
		count.push_back(1);
		threadID = thread;
	}
	inline void AddConfig(InstanceData_sig__Segmentation1LCacc* inst, uint32_t performance, uint32_t cacheMod)
	{
	...
	inline void Run()
	{
		...
		int allocatedAcceleratorCount__segmentation1Mega = 0;
		LCAcc_Request(threadID, 301, 1);//request something of type AcceleratorType
		bool cont = true;
				...
				case(LCACC_GAM_GRANT):
					{
						...
						{
							case(301):
								simics_assert(allocatedAcceleratorCount__segmentation1Mega < 1);
								allocatedAcceleratorIDSet__segmentation1Mega[allocatedAcceleratorCount__segmentation1Mega] = lcaccID;
								allocatedAcceleratorCount__segmentation1Mega++;
								break;
							default:
								simics_assert(0);
						}
						if(allocatedAcceleratorCount__segmentation1Mega == 1)
						{
							cont = false;
						}
					}
		...
		InstanceData_sig__Segmentation1LCacc* inst = NULL;
		for(size_t i = 0; i < instanceSet.size(); i++)
		{
		...
		for(int i = 0; i < 1; i++)
		{
			inst->allocatedAcceleratorIDSet__segmentation1Mega[i] = allocatedAcceleratorIDSet__segmentation1Mega[i];
		}
		inst->allocatedAcceleratorCount__segmentation1Mega = allocatedAcceleratorCount__segmentation1Mega;
		inst->pendingAccelerators = 1;
		StartEverythingHandler_sig__Segmentation1LCacc(inst);
	}
};

#endif
#ifndef LCACC_BODY_TD__Segmentation1LCacc__X
#define LCACC_BODY_TD__Segmentation1LCacc__X

inline void Wait_td__Segmentation1LCacc(int thread)
{
...
inline void CreateBuffer_Segmentation1LCacc_td(uint8_t** buffer, uint32_t* bufferSize, uint8_t** constCluster, int thread, float (*phi), float (*u0), float (*result), int sizeX, int sizeY, int sizeZ, int chunkX, int chunkY, int chunkZ, float dx, float dy, float dz, float lambda1, float lambda2, float mu, float c1, float c2)
{
	simics_assert(chunkX > 0);
	simics_assert(chunkY > 0);
	simics_assert(chunkZ > 0);
	...
	void* LCAcc_FuncArgs__phi = phi;
	void* LCAcc_FuncArgs__u0 = u0;
	void* LCAcc_FuncArgs__result = result;
	{
		void* VNR_vardecl_0(phi);
		void* VNR_vardecl_1(u0);
		void* VNR_vardecl_2(result);
		LCAccNode VNR_vardecl_3(301, (sizeof(float) * (chunkX + 2) * (chunkY + 2) * (chunkZ + 2)) + (sizeof(float) * (chunkX) * (chunkY) * (chunkZ)) + (sizeof(float) * (chunkX) * (chunkY) * (chunkZ)), 2, 0, 0);
		std::vector<uint32_t> VNR_vardecl_4;
		VNR_vardecl_4.push_back(((chunkX + 1) - (1)) / (1));
		VNR_vardecl_4.push_back(((chunkY + 1) - (1)) / (1));
		VNR_vardecl_4.push_back(((chunkZ + 1) - (1)) / (1));
		...
		//Search VNR_vardecl_15 for destination size.
		//Search VNR_vardecl_17 for destination stride.
		mw.AddTransfer(VNR_vardecl_0, VNR_vardecl_13, VNR_vardecl_14, VNR_vardecl_15, VNR_vardecl_16, VNR_vardecl_3, (0) + ((((0) * ((sizeof(float)) * (chunkY + 2) * (chunkZ + 2))) + ((0) * ((sizeof(float)) * (chunkZ + 2))) + ((0) * ((sizeof(float)))))), VNR_vardecl_15, VNR_vardecl_17, sizeof(float));
		...
		//Search VNR_vardecl_18 for destination size.
		//Search VNR_vardecl_19 for destination stride.
		mw.AddTransfer(VNR_vardecl_1, VNR_vardecl_13, VNR_vardecl_14, VNR_vardecl_18, VNR_vardecl_16, VNR_vardecl_3, ((sizeof(float) * (chunkX + 2) * (chunkY + 2) * (chunkZ + 2))) + ((((0) * ((sizeof(float)) * (chunkY) * (chunkZ))) + ((0) * ((sizeof(float)) * (chunkZ))) + ((0) * ((sizeof(float)))))), VNR_vardecl_18, VNR_vardecl_19, sizeof(float));
		...
		//Search VNR_vardecl_7 for destination element size.
		//Search VNR_vardecl_16 for destination element stride.
		mw.AddTransfer(VNR_vardecl_3, ((sizeof(float) * (chunkX + 2) * (chunkY + 2) * (chunkZ + 2)) + (sizeof(float) * (chunkX) * (chunkY) * (chunkZ))) + ((((0) * ((sizeof(float)) * (chunkY) * (chunkZ))) + ((0) * ((sizeof(float)) * (chunkZ))) + ((0) * ((sizeof(float)))))), VNR_vardecl_7, VNR_vardecl_19, VNR_vardecl_2, VNR_vardecl_13, VNR_vardecl_14, VNR_vardecl_7, VNR_vardecl_16, sizeof(float));
...
inline void Segmentation1LCacc_td_buf(uint8_t* buf, uint32_t bufSize, int thread)
{
	...
	Wait_td__Segmentation1LCacc(thread);// wait for everything to finish
}
void Segmentation1LCacc_td(int thread, float (*phi), float (*u0), float (*result), int sizeX, int sizeY, int sizeZ, int chunkX, int chunkY, int chunkZ, float dx, float dy, float dz, float lambda1, float lambda2, float mu, float c1, float c2)
{
	uint32_t bufSize;
	uint8_t* buffer;
	uint8_t* constCluster = NULL;
	CreateBuffer_Segmentation1LCacc_td(&buffer, &bufSize, &constCluster, thread, phi, u0, result, sizeX, sizeY, sizeZ, chunkX, chunkY, chunkZ, dx, dy, dz, lambda1, lambda2, mu, c1, c2);
	Segmentation1LCacc_td_buf(buffer, bufSize, thread);
	if(constCluster)
	{
		delete [] constCluster;
	}
...
inline uint32_t Segmentation1LCacc_CalculateBiNSize(int sizeX, int sizeY, int sizeZ, int chunkX, int chunkY, int chunkZ, float dx, float dy, float dz, float lambda1, float lambda2, float mu, float c1, float c2)
{
	return 0;
}
class BiN_Segmentation1LCacc_Arbitrator_td
{
	...
	InterruptArgs isr;
public:
	inline BiN_Segmentation1LCacc_Arbitrator_td(){}
	...
	inline void Run(int threadID)
	{
		...
		isr.lcaccID = 0;
		isr.lcaccMode = 0;
		LCAcc_DeclareLCAccUse(threadID, 301, 1); //requests for segmentation1Mega
		...
		for(size_t i = 0; i < binSizeSet.size(); i++)
		{
			...
			{
				Segmentation1LCacc_td_buf(bufSet[i], bufSizeSet[i], threadID);
				return;
			}
		}
		simics_assert(0);
