...
#endif
#ifndef LCACC_BODY_SIG__StreamCluster1LCacc__X
#define LCACC_BODY_SIG__StreamCluster1LCacc__X
#define LCACC_CLASS_SIG__StreamCluster1LCacc__x 1
class InstanceData_sig__StreamCluster1LCacc
{
public:
	...
	int allocatedAcceleratorCount__streamCluster1;
	int allocatedAcceleratorIDSet__streamCluster1[1];
	...
	inline void Reset()
	{
		...
		allocatedAcceleratorCount__streamCluster1 = 0;
		reservedAccelerators = 0;
		allocatedAcceleratorIDSet__streamCluster1[0] = 0;
		HandlerArgs__x.threadID = threadID;
		HandlerArgs__x.status = 0;
		HandlerArgs__x.taskIndex = 0;
		HandlerArgs__x.lcaccMode = LCACC_CLASS_SIG__StreamCluster1LCacc__x;
		node_x.Reset();
	}
...
inline void Cleanup_sig__StreamCluster1LCacc(InstanceData_sig__StreamCluster1LCacc* instance);
inline void StartEverythingHandler_sig__StreamCluster1LCacc(InstanceData_sig__StreamCluster1LCacc* instance);
inline void StopEverythingHandler_sig__StreamCluster1LCacc(InstanceData_sig__StreamCluster1LCacc* instance);
inline void ErrorHandler_sig__StreamCluster1LCacc(InstanceData_sig__StreamCluster1LCacc* instance);
inline void Wait_sig__StreamCluster1LCacc(InstanceData_sig__StreamCluster1LCacc* instance);
inline unsigned int DelayEstimate_sig__StreamCluster1LCacc(int vectorSize)
{
	return 1;
}
inline void (*ProgressHandler_sig__StreamCluster1LCacc__x(InstanceData_sig__StreamCluster1LCacc* instance, InterruptArgs* args))(InstanceData_sig__StreamCluster1LCacc*);
//This is a procedurally generated interrupt-based program from a RAGraph data structure for function StreamCluster1LCacc
inline void Wait_sig__StreamCluster1LCacc(InstanceData_sig__StreamCluster1LCacc* instance)
{
	while(1)
	{
		...
		InterruptArgs* args = 0;
		void (*progress)(InstanceData_sig__StreamCluster1LCacc*);
		while((args = LWI_CheckInterrupt(thread)) == 0);
		switch(args->lcaccMode)
		{
			case(0):
				progress = GAMHandler_sig__StreamCluster1LCacc(instance, args);
				break;
			case(LCACC_CLASS_SIG__StreamCluster1LCacc__x):
				progress = ProgressHandler_sig__StreamCluster1LCacc__x(instance, args);
				break;
			default:
				ErrorHandler_sig__StreamCluster1LCacc(instance);
				simics_assert(0);
		}
	...
	switch(args->status)
	{
		case(LCACC_GAM_WAIT):
			...
			if(instance->reservedAccelerators == 1)
			{
				...
				LCAcc_Reserve(args->threadID, 0, DelayEstimate_sig__StreamCluster1LCacc(instance->acceleratorVectorLength));
			}
			return 0;
			break;
		case(LCACC_GAM_GRANT):
			...
			{
				case(1201):
					if(instance->allocatedAcceleratorCount__streamCluster1 == 1)
					{
						return ErrorHandler_sig__StreamCluster1LCacc;
					}
					else
					{
						instance->pendingAccelerators++;
						instance->allocatedAcceleratorIDSet__streamCluster1[instance->allocatedAcceleratorCount__streamCluster1] = lcaccID;
						instance->allocatedAcceleratorCount__streamCluster1++;
					}
					break;
				default:
					return ErrorHandler_sig__StreamCluster1LCacc;
			...
			if(instance->allocatedAcceleratorCount__streamCluster1 == 1)
			{
				return StartEverythingHandler_sig__StreamCluster1LCacc;
			}
			else
			{
				return 0;
...
inline void Cleanup_sig__StreamCluster1LCacc(InstanceData_sig__StreamCluster1LCacc* instance)
{
	...
	if(instance->allocatedAcceleratorIDSet__streamCluster1[0] != 0)
	{
...
inline void StopEverythingHandler_sig__StreamCluster1LCacc(InstanceData_sig__StreamCluster1LCacc* instance)
{
	Cleanup_sig__StreamCluster1LCacc(instance);
}
inline void ErrorHandler_sig__StreamCluster1LCacc(InstanceData_sig__StreamCluster1LCacc* instance)
{
	Cleanup_sig__StreamCluster1LCacc(instance);
...
inline void (*ProgressHandler_sig__StreamCluster1LCacc__x(InstanceData_sig__StreamCluster1LCacc* instance, InterruptArgs* args))(InstanceData_sig__StreamCluster1LCacc*)
{
	switch(args->status)
	{
		...
		case(LCACC_STATUS_COMPLETED):
			...
			{
				return StopEverythingHandler_sig__StreamCluster1LCacc;
			}
			else
			{
				return 0;
...
inline void StartEverythingHandler_sig__StreamCluster1LCacc(InstanceData_sig__StreamCluster1LCacc* instance)
{
	...
	instance->HandlerArgs__x.lcaccID = instance->allocatedAcceleratorIDSet__streamCluster1[0];
	...
	instance->HandlerArgs__x.lcaccMode = LCACC_CLASS_SIG__StreamCluster1LCacc__x;
	...
	Wait_sig__StreamCluster1LCacc(instance);// wait for everything to finish
}
inline void CreateBuffer_StreamCluster1LCacc_sig(int thread, InstanceData_sig__StreamCluster1LCacc* instance, uint32_t (*isCenter), uint32_t (*table), int size, int chunk)
{
	int index, i;
	...
	instance->node_x.SetSPMConfig((sizeof(uint32_t) * (chunk)) + (sizeof(uint32_t) * (chunk)), 3, 0);
	...
	instance->allocatedAcceleratorCount__streamCluster1 = 0;
	...
	VNR_vardecl_4.push_back((1) * (sizeof(uint32_t)));
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_5((0) + ((((0) * (sizeof(uint32_t))))), sizeof(uint32_t), VNR_vardecl_3, VNR_vardecl_4);
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_6(((sizeof(uint32_t) * (chunk))) + ((((0) * (sizeof(uint32_t))))), sizeof(uint32_t), VNR_vardecl_3, VNR_vardecl_4);
	...
	VNR_vardecl_8.push_back(ConvertToType<uint32_t, uint64_t>(0)/*Register counter*/);
	std::vector<uint32_t> VNR_vardecl_9;
	VNR_vardecl_9.push_back(((size) - (0)) / (chunk));
	std::vector<int32_t> VNR_vardecl_10;
	VNR_vardecl_10.push_back((chunk) * ((sizeof(uint32_t))));
	std::vector<int32_t> VNR_vardecl_11;
	VNR_vardecl_11.push_back((1) * ((sizeof(uint32_t))));
	//produce compute set
	...
	instance->acceleratorSignature__x.AddTransfer(VNR_vardecl_1, VNR_vardecl_9, VNR_vardecl_10, VNR_vardecl_3, VNR_vardecl_11, VNR_vardecl_0, (0) + ((((0) * ((sizeof(uint32_t)))))), VNR_vardecl_3, VNR_vardecl_11, sizeof(uint32_t));
	...
	instance->acceleratorSignature__x.AddTransfer(VNR_vardecl_0, ((sizeof(uint32_t) * (chunk))) + ((((0) * ((sizeof(uint32_t)))))), VNR_vardecl_3, VNR_vardecl_11, VNR_vardecl_2, VNR_vardecl_9, VNR_vardecl_10, VNR_vardecl_3, VNR_vardecl_11, sizeof(uint32_t));
	instance->acceleratorSignature__x.Finalize((((size) - (0)) / (chunk)));
}
inline void StreamCluster1LCacc_sig_buf(int threadID, InstanceData_sig__StreamCluster1LCacc* instance)
{
	instance->Reset();
	LWI_RegisterInterruptHandler(&(instance->GAM_INTERACTION));
	LCAcc_Request(threadID, 1201, 1);//request something of type AcceleratorType
	Wait_sig__StreamCluster1LCacc(instance);// wait for everything to finish
}
inline void StreamCluster1LCacc_sig(int threadID, uint32_t (*isCenter), uint32_t (*table), int size, int chunk)
{
	InstanceData_sig__StreamCluster1LCacc instance;
	CreateBuffer_StreamCluster1LCacc_sig(threadID, &instance, isCenter, table, size, chunk);
	StreamCluster1LCacc_sig_buf(threadID, &instance);
}
class BiN_StreamCluster1LCacc_Arbitrator_sig
{
	std::vector<InstanceData_sig__StreamCluster1LCacc*> instanceSet;
	...
	InterruptArgs isr;
public:
	inline BiN_StreamCluster1LCacc_Arbitrator_sig(int thread)
	{
		ops.push_back(1201);
		count.push_back(1);
		threadID = thread;
	}
	inline void AddConfig(InstanceData_sig__StreamCluster1LCacc* inst, uint32_t performance, uint32_t cacheMod)
	{
	...
	inline void Run()
	{
		...
		int allocatedAcceleratorCount__streamCluster1 = 0;
		LCAcc_Request(threadID, 1201, 1);//request something of type AcceleratorType
		bool cont = true;
				...
				case(LCACC_GAM_GRANT):
					{
						...
						{
							case(1201):
								simics_assert(allocatedAcceleratorCount__streamCluster1 < 1);
								allocatedAcceleratorIDSet__streamCluster1[allocatedAcceleratorCount__streamCluster1] = lcaccID;
								allocatedAcceleratorCount__streamCluster1++;
								break;
							default:
								simics_assert(0);
						}
						if(allocatedAcceleratorCount__streamCluster1 == 1)
						{
							cont = false;
						}
					}
		...
		InstanceData_sig__StreamCluster1LCacc* inst = NULL;
		for(size_t i = 0; i < instanceSet.size(); i++)
		{
		...
		for(int i = 0; i < 1; i++)
		{
			inst->allocatedAcceleratorIDSet__streamCluster1[i] = allocatedAcceleratorIDSet__streamCluster1[i];
		}
		inst->allocatedAcceleratorCount__streamCluster1 = allocatedAcceleratorCount__streamCluster1;
		inst->pendingAccelerators = 1;
		StartEverythingHandler_sig__StreamCluster1LCacc(inst);
	}
};

#endif
#ifndef LCACC_BODY_TD__StreamCluster1LCacc__X
#define LCACC_BODY_TD__StreamCluster1LCacc__X

inline void Wait_td__StreamCluster1LCacc(int thread)
{
...
inline void CreateBuffer_StreamCluster1LCacc_td(uint8_t** buffer, uint32_t* bufferSize, uint8_t** constCluster, int thread, uint32_t (*isCenter), uint32_t (*table), int size, int chunk)
{
...
inline void StreamCluster1LCacc_td_buf(uint8_t* buf, uint32_t bufSize, int thread)
{
	...
	Wait_td__StreamCluster1LCacc(thread);// wait for everything to finish
}
void StreamCluster1LCacc_td(int thread, uint32_t (*isCenter), uint32_t (*table), int size, int chunk)
{
	uint32_t bufSize;
	uint8_t* buffer;
	uint8_t* constCluster = NULL;
	CreateBuffer_StreamCluster1LCacc_td(&buffer, &bufSize, &constCluster, thread, isCenter, table, size, chunk);
	StreamCluster1LCacc_td_buf(buffer, bufSize, thread);
	if(constCluster)
	{
		delete [] constCluster;
	}
...
inline uint32_t StreamCluster1LCacc_CalculateBiNSize(int size, int chunk)
{
	return 0;
}
class BiN_StreamCluster1LCacc_Arbitrator_td
{
	...
	InterruptArgs isr;
public:
	inline BiN_StreamCluster1LCacc_Arbitrator_td(){}
	...
	inline void Run(int threadID)
	{
		...
		isr.lcaccID = 0;
		isr.lcaccMode = 0;
		LCAcc_DeclareLCAccUse(threadID, 1201, 1); //requests for streamCluster1
		...
		for(size_t i = 0; i < binSizeSet.size(); i++)
		{
			...
			{
				StreamCluster1LCacc_td_buf(bufSet[i], bufSizeSet[i], threadID);
				return;
			}
		}
		simics_assert(0);
