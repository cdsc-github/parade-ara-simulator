...
inline void CreateBuffer_StreamCluster3LCacc_td(uint8_t** buffer, uint32_t* bufferSize, uint8_t** constCluster, int thread, float (*point)[32], float (*weight), float (*cost), uint32_t (*assign), uint32_t (*in_switch_membership), uint32_t (*out_switch_membership), uint64_t center, uint64_t lower, float xPoint[32], int size, int chunk)
{
	...
	*constCluster = new uint8_t[sizeof(float[32])];
	...
	memcpy(*LCAcc_FuncConst__constant0, xPoint, sizeof(float[32]));
	{
		...
		LCAccNode VNR_vardecl_7(1203, (sizeof(float[32]) * (chunk)) + (sizeof(float[32]) * (1)) + (sizeof(float) * (chunk)) + (sizeof(float) * (chunk)) + (sizeof(uint32_t) * (chunk)) + (sizeof(uint32_t) * (chunk)) + (sizeof(uint32_t) * (chunk)) + (sizeof(uint32_t) * (chunk)) + (sizeof(float) * (chunk)) + (sizeof(uint32_t) * (chunk)), 2, 0, 0);
		std::vector<uint32_t> VNR_vardecl_8;
		VNR_vardecl_8.push_back(((chunk) - (0)) / (1));
		std::vector<int32_t> VNR_vardecl_9;
		VNR_vardecl_9.push_back((1) * (sizeof(float[32])));
