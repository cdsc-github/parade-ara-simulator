...
void TDLCA_StreamCluster::Initialize(int threadID, int procID)
{
    ...
    CreateBuffer_StreamCluster1LCacc_td(&buf1, &buf1Size, &constCluster1, 
        thread, inIsCenter, centerTable, dataSize, 128);
    CreateBuffer_StreamCluster3LCacc_td(&buf3, &buf3Size, &constCluster3, 
        thread, point, weight, inCost, assign, inSwitchMembership, 
        inSwitchMembership, (uint64_t)centerTable, (uint64_t)low, x, 
        dataSize, 128);
    CreateBuffer_StreamCluster4LCacc_td(&buf4, &buf4Size, &constCluster4, 
        thread, inIsCenter, centerTable, low, (uint64_t)globalLow, dataSize, 128);
    CreateBuffer_StreamCluster5LCacc_td(&buf5, &buf5Size, &constCluster5, 
        thread, assign, point, weight, inSwitchMembership, inCost, inCost, 
        (uint64_t) centerTable, (uint64_t) globalLow, x, dataSize, 128);
    CreateBuffer_StreamCluster6LCacc_td(&buf6, &buf6Size, &constCluster6, 
        thread, inIsCenter, centerTable, inIsCenter, (uint64_t) low, dataSize, 128);
    ...
    Touch(thread, weight, dataSize * sizeof(float));
