...
#endif
#ifndef LCACC_BODY_SIG__Swaptions4LCacc__X
#define LCACC_BODY_SIG__Swaptions4LCacc__X
#define LCACC_CLASS_SIG__Swaptions4LCacc__x 1
class InstanceData_sig__Swaptions4LCacc
{
public:
	...
	int allocatedAcceleratorCount__swaptions4;
	int allocatedAcceleratorIDSet__swaptions4[1];
	float (*LCAcc_FuncArgs__pdTotalDrift)[11];
	float (*LCAcc_FuncArgs__shockVals)[55];
	float (*LCAcc_FuncArgs__in_ppdHJMPath)[11][11];
	float (*LCAcc_FuncArgs__out_ppdHJMPath)[11][11];
	float LCAcc_FuncVars__ddelt;
	float LCAcc_FuncVars__sqrt_ddelt;
	...
	inline void Reset()
	{
		...
		allocatedAcceleratorCount__swaptions4 = 0;
		reservedAccelerators = 0;
		allocatedAcceleratorIDSet__swaptions4[0] = 0;
		HandlerArgs__x.threadID = threadID;
		HandlerArgs__x.status = 0;
		HandlerArgs__x.taskIndex = 0;
		HandlerArgs__x.lcaccMode = LCACC_CLASS_SIG__Swaptions4LCacc__x;
		node_x.Reset();
	}
...
inline void Cleanup_sig__Swaptions4LCacc(InstanceData_sig__Swaptions4LCacc* instance);
inline void StartEverythingHandler_sig__Swaptions4LCacc(InstanceData_sig__Swaptions4LCacc* instance);
inline void StopEverythingHandler_sig__Swaptions4LCacc(InstanceData_sig__Swaptions4LCacc* instance);
inline void ErrorHandler_sig__Swaptions4LCacc(InstanceData_sig__Swaptions4LCacc* instance);
inline void Wait_sig__Swaptions4LCacc(InstanceData_sig__Swaptions4LCacc* instance);
inline unsigned int DelayEstimate_sig__Swaptions4LCacc(int vectorSize)
{
	return 1;
}
inline void (*ProgressHandler_sig__Swaptions4LCacc__x(InstanceData_sig__Swaptions4LCacc* instance, InterruptArgs* args))(InstanceData_sig__Swaptions4LCacc*);
//This is a procedurally generated interrupt-based program from a RAGraph data structure for function Swaptions4LCacc
inline void Wait_sig__Swaptions4LCacc(InstanceData_sig__Swaptions4LCacc* instance)
{
	while(1)
	{
		...
		InterruptArgs* args = 0;
		void (*progress)(InstanceData_sig__Swaptions4LCacc*);
		while((args = LWI_CheckInterrupt(thread)) == 0);
		switch(args->lcaccMode)
		{
			case(0):
				progress = GAMHandler_sig__Swaptions4LCacc(instance, args);
				break;
			case(LCACC_CLASS_SIG__Swaptions4LCacc__x):
				progress = ProgressHandler_sig__Swaptions4LCacc__x(instance, args);
				break;
			default:
				ErrorHandler_sig__Swaptions4LCacc(instance);
				simics_assert(0);
		}
	...
	switch(args->status)
	{
		case(LCACC_GAM_WAIT):
			...
			if(instance->reservedAccelerators == 1)
			{
				...
				LCAcc_Reserve(args->threadID, 0, DelayEstimate_sig__Swaptions4LCacc(instance->acceleratorVectorLength));
			}
			return 0;
			break;
		case(LCACC_GAM_GRANT):
			...
			{
				case(1304):
					if(instance->allocatedAcceleratorCount__swaptions4 == 1)
					{
						return ErrorHandler_sig__Swaptions4LCacc;
					}
					else
					{
						instance->pendingAccelerators++;
						instance->allocatedAcceleratorIDSet__swaptions4[instance->allocatedAcceleratorCount__swaptions4] = lcaccID;
						instance->allocatedAcceleratorCount__swaptions4++;
					}
					break;
				default:
					return ErrorHandler_sig__Swaptions4LCacc;
			...
			if(instance->allocatedAcceleratorCount__swaptions4 == 1)
			{
				return StartEverythingHandler_sig__Swaptions4LCacc;
			}
			else
			{
				return 0;
...
inline void Cleanup_sig__Swaptions4LCacc(InstanceData_sig__Swaptions4LCacc* instance)
{
	...
	if(instance->allocatedAcceleratorIDSet__swaptions4[0] != 0)
	{
...
inline void StopEverythingHandler_sig__Swaptions4LCacc(InstanceData_sig__Swaptions4LCacc* instance)
{
	Cleanup_sig__Swaptions4LCacc(instance);
}
inline void ErrorHandler_sig__Swaptions4LCacc(InstanceData_sig__Swaptions4LCacc* instance)
{
	Cleanup_sig__Swaptions4LCacc(instance);
...
inline void (*ProgressHandler_sig__Swaptions4LCacc__x(InstanceData_sig__Swaptions4LCacc* instance, InterruptArgs* args))(InstanceData_sig__Swaptions4LCacc*)
{
	switch(args->status)
	{
		...
		case(LCACC_STATUS_COMPLETED):
			...
			{
				return StopEverythingHandler_sig__Swaptions4LCacc;
			}
			else
			{
				return 0;
...
inline void StartEverythingHandler_sig__Swaptions4LCacc(InstanceData_sig__Swaptions4LCacc* instance)
{
	...
	instance->HandlerArgs__x.lcaccID = instance->allocatedAcceleratorIDSet__swaptions4[0];
	...
	instance->HandlerArgs__x.lcaccMode = LCACC_CLASS_SIG__Swaptions4LCacc__x;
	...
	Wait_sig__Swaptions4LCacc(instance);// wait for everything to finish
}
inline void CreateBuffer_Swaptions4LCacc_sig(int thread, InstanceData_sig__Swaptions4LCacc* instance, float (*pdTotalDrift)[11], float (*shockVals)[55], float (*in_ppdHJMPath)[11][11], float (*out_ppdHJMPath)[11][11], float ddelt, float sqrt_ddelt, int size, int chunk)
{
	int index, i;
	...
	instance->node_x.SetSPMConfig((sizeof(float[11]) * (chunk)) + (sizeof(float[55]) * (chunk)) + (sizeof(float[11][11]) * (chunk)) + (sizeof(float[11][11]) * (chunk)), 3, 0);
	instance->LCAcc_FuncArgs__pdTotalDrift = pdTotalDrift;
	instance->LCAcc_FuncArgs__shockVals = shockVals;
	instance->LCAcc_FuncArgs__in_ppdHJMPath = in_ppdHJMPath;
	instance->LCAcc_FuncArgs__out_ppdHJMPath = out_ppdHJMPath;
	instance->LCAcc_FuncVars__ddelt = ddelt;
	instance->LCAcc_FuncVars__sqrt_ddelt = sqrt_ddelt;
	instance->LCAcc_FuncVars__size = size;
	instance->LCAcc_FuncVars__chunk = chunk;
	instance->allocatedAcceleratorCount__swaptions4 = 0;
	LCAccNode& VNR_vardecl_0(instance->node_x);
	void* VNR_vardecl_1(instance->LCAcc_FuncArgs__pdTotalDrift);
	void* VNR_vardecl_2(instance->LCAcc_FuncArgs__shockVals);
	void* VNR_vardecl_3(instance->LCAcc_FuncArgs__in_ppdHJMPath);
	void* VNR_vardecl_4(instance->LCAcc_FuncArgs__out_ppdHJMPath);
	std::vector<uint32_t> VNR_vardecl_5;
	VNR_vardecl_5.push_back(((chunk) - (0)) / (1));
	std::vector<int32_t> VNR_vardecl_6;
	VNR_vardecl_6.push_back((1) * (sizeof(float[11])));
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_7((0) + ((((0) * (sizeof(float[11]))))), sizeof(float), VNR_vardecl_5, VNR_vardecl_6);
	std::vector<int32_t> VNR_vardecl_8;
	VNR_vardecl_8.push_back((1) * (sizeof(float[55])));
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_9(((sizeof(float[11]) * (chunk))) + ((((0) * (sizeof(float[55]))))), sizeof(float), VNR_vardecl_5, VNR_vardecl_8);
	std::vector<int32_t> VNR_vardecl_10;
	VNR_vardecl_10.push_back((1) * (sizeof(float[11][11])));
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_11(((sizeof(float[11]) * (chunk)) + (sizeof(float[55]) * (chunk))) + ((((0) * (sizeof(float[11][11]))))), sizeof(float), VNR_vardecl_5, VNR_vardecl_10);
	MicroprogramWriter::ComputeArgIndex VNR_vardecl_12(((sizeof(float[11]) * (chunk)) + (sizeof(float[55]) * (chunk)) + (sizeof(float[11][11]) * (chunk))) + ((((0) * (sizeof(float[11][11]))))), sizeof(float), VNR_vardecl_5, VNR_vardecl_10);
	std::vector<MicroprogramWriter::ComputeArgIndex> VNR_vardecl_13;
	VNR_vardecl_13.push_back(VNR_vardecl_7);
	VNR_vardecl_13.push_back(VNR_vardecl_9);
	VNR_vardecl_13.push_back(VNR_vardecl_11);
	VNR_vardecl_13.push_back(VNR_vardecl_12);
	std::vector<uint64_t> VNR_vardecl_14;
	VNR_vardecl_14.push_back(ConvertToType<float, uint64_t>(ddelt)/*Register ddelt*/);
	VNR_vardecl_14.push_back(ConvertToType<float, uint64_t>(sqrt_ddelt)/*Register sqrt_ddelt*/);
	std::vector<uint32_t> VNR_vardecl_15;
	VNR_vardecl_15.push_back(((size) - (0)) / (chunk));
	std::vector<int32_t> VNR_vardecl_16;
	VNR_vardecl_16.push_back((chunk) * ((sizeof(float[11]))));
	std::vector<uint32_t> VNR_vardecl_17;
	VNR_vardecl_17.push_back(((chunk) - (0)) / (1));
	VNR_vardecl_17.push_back((11));
	std::vector<int32_t> VNR_vardecl_18;
	VNR_vardecl_18.push_back((1) * ((sizeof(float[11]))));
	VNR_vardecl_18.push_back((sizeof(float)));
	std::vector<int32_t> VNR_vardecl_19;
	VNR_vardecl_19.push_back((chunk) * ((sizeof(float[55]))));
	std::vector<uint32_t> VNR_vardecl_20;
	VNR_vardecl_20.push_back(((chunk) - (0)) / (1));
	VNR_vardecl_20.push_back((55));
	std::vector<int32_t> VNR_vardecl_21;
	VNR_vardecl_21.push_back((1) * ((sizeof(float[55]))));
	VNR_vardecl_21.push_back((sizeof(float)));
	std::vector<int32_t> VNR_vardecl_22;
	VNR_vardecl_22.push_back((chunk) * ((sizeof(float[11][11]))));
	std::vector<uint32_t> VNR_vardecl_23;
	VNR_vardecl_23.push_back(((chunk) - (0)) / (1));
	VNR_vardecl_23.push_back((121));
	std::vector<int32_t> VNR_vardecl_24;
	VNR_vardecl_24.push_back((1) * ((sizeof(float[11][11]))));
	VNR_vardecl_24.push_back((sizeof(float)));
	//produce compute set
	//See VNR_vardecl_13 for index variable decl
	//See VNR_vardecl_14 for register set decl
	instance->acceleratorSignature__x.AddCompute(VNR_vardecl_0, VNR_vardecl_13, VNR_vardecl_14);
	...
	instance->acceleratorSignature__x.AddTransfer(VNR_vardecl_1, VNR_vardecl_15, VNR_vardecl_16, VNR_vardecl_17, VNR_vardecl_18, VNR_vardecl_0, (0) + ((((0) * ((sizeof(float[11])))))), VNR_vardecl_17, VNR_vardecl_18, sizeof(float));
	//transfer from shockVals to x
	//Search VNR_vardecl_15 for source block size.
	//Search VNR_vardecl_19 for source block stride.
	//Search VNR_vardecl_20 for source element size.
	//Search VNR_vardecl_21 for source element stride.
	//Search VNR_vardecl_20 for destination size.
	//Search VNR_vardecl_21 for destination stride.
	instance->acceleratorSignature__x.AddTransfer(VNR_vardecl_2, VNR_vardecl_15, VNR_vardecl_19, VNR_vardecl_20, VNR_vardecl_21, VNR_vardecl_0, ((sizeof(float[11]) * (chunk))) + ((((0) * ((sizeof(float[55])))))), VNR_vardecl_20, VNR_vardecl_21, sizeof(float));
	//transfer from in_ppdHJMPath to x
	//Search VNR_vardecl_15 for source block size.
	//Search VNR_vardecl_22 for source block stride.
	//Search VNR_vardecl_23 for source element size.
	...
	instance->acceleratorSignature__x.AddTransfer(VNR_vardecl_3, VNR_vardecl_15, VNR_vardecl_22, VNR_vardecl_23, VNR_vardecl_24, VNR_vardecl_0, ((sizeof(float[11]) * (chunk)) + (sizeof(float[55]) * (chunk))) + ((((0) * ((sizeof(float[11][11])))))), VNR_vardecl_23, VNR_vardecl_24, sizeof(float));
	...
	instance->acceleratorSignature__x.AddTransfer(VNR_vardecl_0, ((sizeof(float[11]) * (chunk)) + (sizeof(float[55]) * (chunk)) + (sizeof(float[11][11]) * (chunk))) + ((((0) * ((sizeof(float[11][11])))))), VNR_vardecl_23, VNR_vardecl_24, VNR_vardecl_4, VNR_vardecl_15, VNR_vardecl_22, VNR_vardecl_23, VNR_vardecl_24, sizeof(float));
	instance->acceleratorSignature__x.Finalize((((size) - (0)) / (chunk)));
}
inline void Swaptions4LCacc_sig_buf(int threadID, InstanceData_sig__Swaptions4LCacc* instance)
{
	instance->Reset();
	LWI_RegisterInterruptHandler(&(instance->GAM_INTERACTION));
	LCAcc_Request(threadID, 1304, 1);//request something of type AcceleratorType
	Wait_sig__Swaptions4LCacc(instance);// wait for everything to finish
}
inline void Swaptions4LCacc_sig(int threadID, float (*pdTotalDrift)[11], float (*shockVals)[55], float (*in_ppdHJMPath)[11][11], float (*out_ppdHJMPath)[11][11], float ddelt, float sqrt_ddelt, int size, int chunk)
{
	InstanceData_sig__Swaptions4LCacc instance;
	CreateBuffer_Swaptions4LCacc_sig(threadID, &instance, pdTotalDrift, shockVals, in_ppdHJMPath, out_ppdHJMPath, ddelt, sqrt_ddelt, size, chunk);
	Swaptions4LCacc_sig_buf(threadID, &instance);
}
class BiN_Swaptions4LCacc_Arbitrator_sig
{
	std::vector<InstanceData_sig__Swaptions4LCacc*> instanceSet;
	...
	InterruptArgs isr;
public:
	inline BiN_Swaptions4LCacc_Arbitrator_sig(int thread)
	{
		ops.push_back(1304);
		count.push_back(1);
		threadID = thread;
	}
	inline void AddConfig(InstanceData_sig__Swaptions4LCacc* inst, uint32_t performance, uint32_t cacheMod)
	{
	...
	inline void Run()
	{
		...
		int allocatedAcceleratorCount__swaptions4 = 0;
		LCAcc_Request(threadID, 1304, 1);//request something of type AcceleratorType
		bool cont = true;
				...
				case(LCACC_GAM_GRANT):
					{
						...
						{
							case(1304):
								simics_assert(allocatedAcceleratorCount__swaptions4 < 1);
								allocatedAcceleratorIDSet__swaptions4[allocatedAcceleratorCount__swaptions4] = lcaccID;
								allocatedAcceleratorCount__swaptions4++;
								break;
							default:
								simics_assert(0);
						}
						if(allocatedAcceleratorCount__swaptions4 == 1)
						{
							cont = false;
						}
					}
		...
		InstanceData_sig__Swaptions4LCacc* inst = NULL;
		for(size_t i = 0; i < instanceSet.size(); i++)
		{
		...
		for(int i = 0; i < 1; i++)
		{
			inst->allocatedAcceleratorIDSet__swaptions4[i] = allocatedAcceleratorIDSet__swaptions4[i];
		}
		inst->allocatedAcceleratorCount__swaptions4 = allocatedAcceleratorCount__swaptions4;
		inst->pendingAccelerators = 1;
		StartEverythingHandler_sig__Swaptions4LCacc(inst);
	}
};

#endif
#ifndef LCACC_BODY_TD__Swaptions4LCacc__X
#define LCACC_BODY_TD__Swaptions4LCacc__X

inline void Wait_td__Swaptions4LCacc(int thread)
{
...
inline void CreateBuffer_Swaptions4LCacc_td(uint8_t** buffer, uint32_t* bufferSize, uint8_t** constCluster, int thread, float (*pdTotalDrift)[11], float (*shockVals)[55], float (*in_ppdHJMPath)[11][11], float (*out_ppdHJMPath)[11][11], float ddelt, float sqrt_ddelt, int size, int chunk)
{
	MicroprogramWriter mw(false);
	void* LCAcc_FuncArgs__pdTotalDrift = pdTotalDrift;
	void* LCAcc_FuncArgs__shockVals = shockVals;
	void* LCAcc_FuncArgs__in_ppdHJMPath = in_ppdHJMPath;
	void* LCAcc_FuncArgs__out_ppdHJMPath = out_ppdHJMPath;
	{
		void* VNR_vardecl_0(pdTotalDrift);
...
inline void Swaptions4LCacc_td_buf(uint8_t* buf, uint32_t bufSize, int thread)
{
	...
	Wait_td__Swaptions4LCacc(thread);// wait for everything to finish
}
void Swaptions4LCacc_td(int thread, float (*pdTotalDrift)[11], float (*shockVals)[55], float (*in_ppdHJMPath)[11][11], float (*out_ppdHJMPath)[11][11], float ddelt, float sqrt_ddelt, int size, int chunk)
{
	uint32_t bufSize;
	uint8_t* buffer;
	uint8_t* constCluster = NULL;
	CreateBuffer_Swaptions4LCacc_td(&buffer, &bufSize, &constCluster, thread, pdTotalDrift, shockVals, in_ppdHJMPath, out_ppdHJMPath, ddelt, sqrt_ddelt, size, chunk);
	Swaptions4LCacc_td_buf(buffer, bufSize, thread);
	if(constCluster)
	{
		delete [] constCluster;
	}
...
inline uint32_t Swaptions4LCacc_CalculateBiNSize(float ddelt, float sqrt_ddelt, int size, int chunk)
{
	return 0;
}
class BiN_Swaptions4LCacc_Arbitrator_td
{
	...
	InterruptArgs isr;
public:
	inline BiN_Swaptions4LCacc_Arbitrator_td(){}
	...
	inline void Run(int threadID)
	{
		...
		isr.lcaccID = 0;
		isr.lcaccMode = 0;
		LCAcc_DeclareLCAccUse(threadID, 1304, 1); //requests for swaptions4
		...
		for(size_t i = 0; i < binSizeSet.size(); i++)
		{
			...
			{
				Swaptions4LCacc_td_buf(bufSet[i], bufSizeSet[i], threadID);
				return;
			}
		}
		simics_assert(0);
