...
void TDLCA_Swaptions::Initialize(int threadID, int procID)
{
	...
	CreateBuffer_Swaptions1LCacc_td(&buf1, &buf1Size, &constCluster1, thread, randSeeds, randVals, dataSize, 32);
	CreateBuffer_Swaptions2LCacc_td(&buf2, &buf2Size, &constCluster2, thread, (float*)randVals, (float*)pdZ, dataSize, 3, 11, 32);
	CreateBuffer_Swaptions3LCacc_td(&buf3, &buf3Size, &constCluster3, thread, ppdFactors, pdZ, shock, dataSize, 32);
	CreateBuffer_Swaptions4LCacc_td(&buf4, &buf4Size, &constCluster4, thread, pdTotalDrift, shock, in_ppdHJMPath, out_ppdHJMPath, 0.0f, 0.0f, dataSize, 32);
