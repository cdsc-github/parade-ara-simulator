...
{       
  //std::cout << "[LWI_RegisterInterruptHandler]=====================" << std::endl;
	...
	for(int i = 0; i < max;)//i is incremented, or not as in the case of detected TLB misses, by the MAGIC instruciton below
        {
		//std::cout << "[LWI_RegisterInterruptHandler]MAGIC_CMD(MAGIC_LWI_REGISTER, " << threadID<< ", " << lcacc << ", " << argPtr << ", " << i << ", " << max << ", 0, 0)" << std::endl; // Set ISR address
		//std::cout << "[LWI_RegisterInterruptHandler]InterruptArgs[" << argPtr << "]" << argPtr->status << ", " << argPtr->threadID << ", " << argPtr->lcaccID << ", " << argPtr->taskIndex << ", " << argPtr->lcaccMode << ", " << argPtr->replicate << std::endl;
                MAGIC_CMD(i, MAGIC_LWI_REGISTER, threadID, lcacc, argPtr, i, max,0,0); // Set ISR address
		//std::cout << "[LWI_RegisterInterruptHandler]Returned " << i << std::endl;
        }
}
...
void LCAcc_Command(uint32_t threadID, int lcaccID, int32_t action, void* addr, int32_t arg1, int32_t arg2, int32_t arg3)
{
        ...
        while(!finished)
        {
			...
			*dummy = dummy2;
                ...
                MAGIC_CMD(finished, MAGIC_LCACC_COMMAND, threadID, lcaccID, action, reinterpret_cast<uint64_t>(addr), arg1, arg2, arg3); //set return addresses
        }
}
...
/*void KillThread()
{
        uint64_t ret;
        //MAGIC_CMD(0xCCCD);
        MAGIC_CMD(ret, 0xCCCD,0,0,0,0,0,0,0);
}//*/
void KillSimulation()
{
        //MAGIC_CMD(4 << 16);
...
void BarrierWait(int t, int id)
{
        ...
        while(x)
        {
                ...
                //__asm__ __volatile__("lduw %0, %%l4;"  :  :"m"(id) : "%l4");
                //MAGIC_CMD(0xBA01);
                //__asm__ __volatile__("stuw %%l3, %0;"  :  :"m"(x) : "%l3");
        }
        for(x = 0; x < 10; x++);
}
void simics_break()
...
void simics_debug(uint32_t thread, uint32_t marker)
{
        uint64_t ret;
        //__asm__ __volatile__("lduw %0, %%l4;"  :  :"m"(thread) : "%l4");
        //__asm__ __volatile__("lduw %0, %%l5;"  :  :"m"(marker) : "%l5");
        //MAGIC_CMD(0xCCCE);
        ...
        for(int i = 0; i < size; i += 4096)
        {
		  ...
		  //enforce OS to allocate a physical page and setup the page table
			...
			*(a + i) = trashVal;
                ...
                //__asm__ __volatile__("lduw %0, %%l3;"  :  :"m"(thread) : "%l3");
                //__asm__ __volatile__("lduw %0, %%l4;"  :  :"m"(a + i) : "%l4");
                //MAGIC_CMD(0xC101);
                MAGIC_CMD(ret, 0xC101, thread, (a + i), 0,0,0,0,0);
        }

	//Also need to touch the last byte, 
	//previous loop may miss the last page
	//(unaligned case) because the step is 4096
	uint8_t trashVal = *(a + size - 1);
	*(a + size - 1) = trashVal;
	MAGIC_CMD(ret, 0xC101, thread, (a + size - 1), 0,0,0,0,0);
}
