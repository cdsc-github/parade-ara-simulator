...
WARMUP=(
###Medical Imaging(MI) benchmarks##############
    '47213458' #'Deblur_Modified'
    '45739511' #'Denoise'
    '96520778' #'Registration_Modified'
    '35536286' #'Segmentation'
###CoMmercial(CM) benchmarks###################
    '16986239' #'BlackScholes'
    '25836754' #'StreamCluster'
    '20416917' #'Swaptions'
###computer VISion(VIS) benchmarks#############
    '98502585' #'LPCIP_Desc'
    '31302339' #'Texture_Synthesis'
###NAVigation(NAV) benchmarks##################
    '40640553' #'Robot_Localization'
    '17925138' #'Disparity_Map'
    '61292080' #'EKF_SLAM'
)
...
for((i=0; i < ${#BENCHS[@]}; i++)) do
    ...
    GEM5=${BENCH_DIR}/gem5.opt
    echo "(time -p ${GEM5} --outdir=${BENCH_DIR} ${COMMON_CONFIG} --accelerators=${NUM_ACCELERATORS[$i]} -s ${WARMUP[$i]} -W ${WARMUP[$i]} --script=${SCRIPT_FILE}) >& ${OUT_FILE} &"
    (time -p ${GEM5} --outdir=${BENCH_DIR} ${COMMON_CONFIG} --accelerators=${NUM_ACCELERATORS[$i]} -s ${WARMUP[$i]} -W ${WARMUP[$i]} --script=${SCRIPT_FILE}) >& ${OUT_FILE} &
done
