#ifndef LCACC_MODE_JACOBIANS_DIMM_H
#define LCACC_MODE_JACOBIANS_DIMM_H
...
namespace LCAcc
{
	class OperatingMode_Jacobians_dimm : public LCAccOperatingMode
	{
	public:
		inline OperatingMode_Jacobians_dimm(){}
		...
		inline virtual void GetSPMWriteIndexSet(int iteration, int maxIteration, int taskID, const std::vector<uint64_t>& argAddrVec, const std::vector<bool>& argActive, std::vector<uint64_t>& outputArgs)
		{
			...
			uint64_t addr_df_dx_1 = argAddrVec[2];
			assert(3 < argAddrVec.size());
			uint64_t addr_df_dx_2 = argAddrVec[3];
			assert(4 < argAddrVec.size());
			uint64_t addr_df_dx_3 = argAddrVec[4];
			assert(5 < argAddrVec.size());
			uint64_t addr_df_dx_4 = argAddrVec[5];
			assert(6 < argAddrVec.size());
			uint64_t addr_X_oplus_U = argAddrVec[6];

			if(argActive[2])
			{
				for(size_t i = 0; i < ((7)) * GetArgumentWidth(2); i += GetArgumentWidth(2))
				{
					outputArgs.push_back(addr_df_dx_1 + i);
				}
			}
			if(argActive[3])
			{
				...
				{
					outputArgs.push_back(addr_df_dx_2 + i);
				}
			}
			if(argActive[4])
			{
				for(size_t i = 0; i < ((7)) * GetArgumentWidth(4); i += GetArgumentWidth(4))
				{
					outputArgs.push_back(addr_df_dx_3 + i);
				}
			}
			if(argActive[5])
			{
				for(size_t i = 0; i < ((7)) * GetArgumentWidth(5); i += GetArgumentWidth(5))
				{
					outputArgs.push_back(addr_df_dx_4 + i);
				}
			}
			if(argActive[6])
			{
				for(size_t i = 0; i < ((7)) * GetArgumentWidth(6); i += GetArgumentWidth(6))
				{
		...
		inline virtual void Compute(int iteration, int maxIteration, int taskID, const std::vector<uint64_t>& LCACC_INTERNAL_argAddrVec, const std::vector<bool>& LCACC_INTERNAL_argActive)
		{
			...
			uint64_t addr_df_dx_1 = LCACC_INTERNAL_argAddrVec[2];
			assert(3 < LCACC_INTERNAL_argAddrVec.size());
			uint64_t addr_df_dx_2 = LCACC_INTERNAL_argAddrVec[3];
			assert(4 < LCACC_INTERNAL_argAddrVec.size());
			uint64_t addr_df_dx_3 = LCACC_INTERNAL_argAddrVec[4];
			assert(5 < LCACC_INTERNAL_argAddrVec.size());
			uint64_t addr_df_dx_4 = LCACC_INTERNAL_argAddrVec[5];
			assert(6 < LCACC_INTERNAL_argAddrVec.size());
			uint64_t addr_X_oplus_U = LCACC_INTERNAL_argAddrVec[6];
			...
			for(int i = 0; i < ((7)); i++)
			{
				df_dx_1[(i) % (7)] = (double)0;
			}
			for(int i = 0; i < ((7)); i++)
			{
				df_dx_2[(i) % (7)] = (double)0;
			}
			for(int i = 0; i < ((7)); i++)
			{
				df_dx_3[(i) % (7)] = (double)0;
			}
			for(int i = 0; i < ((7)); i++)
			{
				df_dx_4[(i) % (7)] = (double)0;
			}
			for(int i = 0; i < ((7)); i++)
			{
			...
			float df_dx[28];//to preserve the old variable naming
			...
			for(int i = 0; i < 7; i++)
			{//reconstruct the sliced up variable
				df_dx_1[i] = df_dx[i];
				df_dx_2[i] = df_dx[7 + i];
				df_dx_3[i] = df_dx[14 + i];
				df_dx_4[i] = df_dx[21 + i];
			}
			#undef SPMAddressOf

			for(size_t i = 0; i < ((7)); i++)
			{
				WriteSPMFlt(2, addr_df_dx_1, i, df_dx_1[(i) % (7)]);
			}
			for(size_t i = 0; i < ((7)); i++)
			{
				WriteSPMFlt(3, addr_df_dx_2, i, df_dx_2[(i) % (7)]);
			}
			for(size_t i = 0; i < ((7)); i++)
			{
				WriteSPMFlt(4, addr_df_dx_3, i, df_dx_3[(i) % (7)]);
			}
			for(size_t i = 0; i < ((7)); i++)
			{
				WriteSPMFlt(5, addr_df_dx_4, i, df_dx_4[(i) % (7)]);
			}
			for(size_t i = 0; i < ((7)); i++)
			{
				WriteSPMFlt(6, addr_X_oplus_U, i, X_oplus_U[(i) % (7)]);
			}
		}
