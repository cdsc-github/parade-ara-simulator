...
LCAccDevice::EmitActionReads(Action* a)
{
  ...
  if (a->readSet.size() > 0) {
    ...
    for (size_t i = 0; i < a->readSet.size(); i++) {
      ...
      dma->BeginTransfer(t.src.spm, (uint64_t)t.src.addr, t.src.sizeSet,
        t.src.strideSet, t.dst.spm, (uint64_t)t.dst.addr, t.dst.sizeSet,
        t.dst.strideSet, t.elementSize, Priority::Medium,
  ...
  while (!awaitingComputeActionSet.empty() &&
    ...
    if (ca->compute) {
      ...
      for (size_t i = 0; i < ca->compute->computeCount; i++) {
        ...
        ca->compute->mode->GetSPMReadIndexSet(i, ca->compute->maxCompute,
          ca->taskID, seedAddrs, ca->compute->argumentActive, readBufferAddrs);
...
LCAccDevice::IssueComputeElement(ComputeElement* ce)
{
  ...
  uint64_t startOffset =
    ...
    (nextComputeAvailable - gem5Interface::GetSystemTime()) : 0;
...
LCAccDevice::PerformComputeElement(ComputeElement* ce)
{
  ...
  ce->host->mode->Compute(ce->index, ce->maxCompute, ce->taskID, seedAddrs,
    ce->host->argumentActive);
  std::vector<uint64_t> writeAddrs;
  ce->host->mode->GetSPMWriteIndexSet(ce->index, ce->maxCompute, ce->taskID,
    seedAddrs, ce->host->argumentActive, writeAddrs);
  ...
  if (writeAddrs.size() > 0 || ce->randomAccessesRemaining > 0) {
    ...
    for (size_t i = 0; i < ce->randomAccessesRemaining; i++) {
      ...
      dma->BeginSingleElementTransfer(spm->GetID(), spmAddr, memAddr, size, type,
...
void
LCAccDevice::ComputeRandomAccessComplete(ComputeElement* ce, int iteration,
  int maxIteration, int taskID, uint64_t spmAddr, uint64_t memAddr,
  int accessType)
{
  assert(ce);
  ...
  ce->host->mode->MemoryAccessComplete(iteration, maxIteration, taskID,
    spmAddr, memAddr, accessType, seedAddrs);
...
void
LCAccDevice::MsgHandler(int src, const void* packet, unsigned int packetSize)
{
...
void
LCAccDevice::ReadTaskSignatureDone()
{
...
LCAccDevice::ParseTaskSignature(PacketReader& pr)
{
  ...
  if (pr.SizeRemaining() > 2 * sizeof(uint32_t) + sizeof(uint64_t)) {
    ...
    ML_LOG(GetDeviceName(), "preload TLB with " << tlbCounter << " entries");

    for (uint32_t i = 0; i < tlbCounter; i++) {
      uint32_t logicalPage = pr.Read<uint32_t>();
      uint64_t physicalPage = pr.Read<uint64_t>();
      ML_LOG(GetDeviceName(), "0x" << std::hex << logicalPage << " -> 0x"
        << std::hex << physicalPage);
  ...
  for (unsigned int i = 0; i < computesPerTask; i++) {
    ...
    computeSet.push_back(ComputeOrder(modes[cd.opcode], computeSize,
      computeSet.size(), (computeSet.size() % spmWindowCount) * spmWindowSize,
...
LCAccDevice::StartTaskRead()
{
  ...
  dma->BeginTransfer(NO_SPM_ID, t.startingAddress, size, stride, spm->GetID(),
    spmTaskLoadAddr, size, stride, 1, Priority::Low,
    ReadTaskSignatureDoneCB::Create(this));
}

void
LCAccDevice::HandleTLBMiss(uint64_t addr)
{
  uint32_t msg[4];
...
void
LCAccDevice::HandleAccessViolation(uint64_t)
{
...
void
LCAccDevice::AddNetworkInterface(NetworkInterface* port)
{
  assert(netPort == NULL);
  netPort = port;
}
...
void
LCAccDevice::AddProgrammablePrefetcher(int networkID)
{
  prefetcherSet.push_back(networkID);
}

void
LCAccDevice::CalculateSPMMapping(int bankCount, int bankMod, int readPorts,
  int writePorts, const ComputeDescription& compute, std::map<unsigned int,
...
LCAccDevice::Initialize(uint32_t threadID)
{
  ...
  spm = new SPMInterface(GetDeviceName(), netPort->GetNodeID(),
    netPort->GetNetworkPort(), 1, spmBanks, spmBankMod, spmReadPortCount,
    spmReadPortLatency, spmWritePortCount, spmWritePortLatency);
  ...
  gem5Interface::RegisterLCAcc(netPort->GetNetworkPort(),
    netPort->GetNodeID(), m, threadID);
