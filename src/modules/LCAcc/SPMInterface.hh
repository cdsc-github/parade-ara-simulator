...
namespace LCAcc
{
    class SPMInterface
    {
        ...
        int bankMod;
        // "Slots" reference sub-cycle read/write allocations.  Each 'slot'
        // refers to a port per cycle.  This is used to track the number of
        // accesses that have occured during the current, and future cycles.
        // The number here represents <cycle> * <port count> + <port>, and
        // indicates the most recently allocated port for a given cycle.  When
        // an access is performed, this value is read and compared against
        // <cycle> * <port count>.  If it is smaller, then it is set to
        // <cycle> * <port count>, and the access occurs during this cycle. If
        // it is equal or larger, it is set to <old value> + 1, and the access
        // is initiated at <old value> / <port count>, which may be a time in
        // the future.  This tracks the ordering of accesses without needing to
        // maintain an explicit queue. or re-try accesses. There is one of
        // these values per bank.
        std::vector<unsigned long long> readSlot;
    ...
    public:
        SPMInterface(const std::string& hostName, int cpuPort, int id,
            int size, int banks, int bankMod, int readPorts, int readLatency,
            int writePorts, int writeLatency);
        ~SPMInterface();
