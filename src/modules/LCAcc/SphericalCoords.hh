...
#define opt_invSqrt_f_f(FLOAT_IN, FLOAT_OUT) { \
    ...
    // inverseComposePoint();
    {
      ...
      // Function itself:
      const float t2 = -qr * qx;
      const float t3 = -qr * qy;
      const float t4 = -qr * qz;
      const float t5 = -qx * qx;
      const float t6 =  qx * qy;
      const float t7 =  qx * qz;
      const float t8 = -qy * qy;
      const float t9 =  qy * qz;
      const float t10 = -qz * qz;
