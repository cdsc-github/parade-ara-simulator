#ifndef LCACC_MODE_FFT_DIMM_H
#define LCACC_MODE_FFT_DIMM_H
...
namespace LCAcc
{
	class OperatingMode_fft_dimm : public LCAccOperatingMode
	{
		int elemCount;
	public:
		inline OperatingMode_fft_dimm(){}
		...
		inline static std::string GetModeName(){return "fft_dimm";}
		inline virtual int ArgumentCount(){return 2;}
		inline virtual void SetRegisterValues(const std::vector<uint64_t>& regs)
		{
			assert(regs.size() == 1);
			elemCount = ConvertTypes<uint32_t, int>(regs[0]);
		}
		inline static int GetOpCode(){return 751;}
	};
}
