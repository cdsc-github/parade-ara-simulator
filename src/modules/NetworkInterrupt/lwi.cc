...
extern "C"
{
  ...
  void raiseLightWeightInt(int thread, void* args, int argSize, int interrupting_lcacc)
  {
    ...
    ML_LOG("magicintercept", "interrupt raised for thread " << thread
      << " from lcacc " << interrupting_lcacc);
...
#ifdef SIM_ENHANCE
logical_address_t
LWI_FetchLogicalAddr(int thread, int index)
{
...
void
LWI_RegisterAccepter(int thread, int lcacc, physical_address_t pa,
  logical_address_t la, int index)
{
  if (index == 0) {
    ML_LOG("magicintercept", "registering accepter for thread " << thread
      << " to service lcacc " << lcacc);
    LWI_pendingAccepters[thread][lcacc] =
      LWI_MessageAccepter(thread, lcacc, pa, la);
  } else {
...
void
LWI_UnregisterAccepter(int thread, int lcacc)
{
...
bool
LWI_GetMessagePair(int thread, LWI_StoredMessage& msg, LWI_MessageAccepter& accepter)
{
  if (LWI_inInterrupt.find(thread) != LWI_inInterrupt.end()) {
    return false;
  }
  ...
  ML_LOG("magicintercept", "BEGIN Interrupt Handling "
    << LWI_pendingMessages[thread].lcacc);
...
void
LWI_EndInterruptHandling(int thread)
{
  assert(LWI_pendingMessages.find(thread) != LWI_pendingMessages.end());
  assert(LWI_pendingAccepters.find(thread) != LWI_pendingAccepters.end());
  assert(LWI_pendingAccepters[thread].find(LWI_pendingMessages[thread].lcacc) != LWI_pendingAccepters[thread].end());
  ...
  ML_LOG("magicintercept", "END Interrupt Handling "
    << LWI_pendingMessages[thread].lcacc);
