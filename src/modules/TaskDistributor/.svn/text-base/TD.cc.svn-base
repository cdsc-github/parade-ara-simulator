...
void TD::JobDescription::WriteJobSegment(int logicalCFU, const std::map<int, int>& logicalToPhysCFUs, PacketBuilder& pb) const
{
	uint32_t transferCount = hostProgram->nodeSet[logicalCFU].outgoingEdges.size() + hostProgram->nodeSet[logicalCFU].incomingEdges.size();
	...
	{
		const TDProgram::Edge& e = hostProgram->edgeSet[hostProgram->nodeSet[logicalCFU].incomingEdges[i]];
		int src, dst;
		if(e.from == -1)
		{
			src = -1;
	...
	{
		const TDProgram::Edge& e = hostProgram->edgeSet[hostProgram->nodeSet[logicalCFU].outgoingEdges[i]];
		int src, dst;
		if(e.from == -1)
		{
			src = -1;
...
std::vector<uint32_t> TD::ExtractMemoryManifest(const TransferDescription& td, uint32_t task)
{
...
bool TD::TryAllocateFpga(std::vector<CFUIdentifier>& cfuSet, const std::vector<int>& opCodeSet, int minLatency, int maxLatency, bool probeAllocate)
{
	...
	for(size_t x = 0; x < opCodeSet.size(); x++)
	{
		...
		{
			if(fpgaRecipeSet[i].opCode == opCodeSet[x] && fpgaRecipeSet[i].ii * fpgaRecipeSet[i].clockMultiplier >= minLatency && fpgaRecipeSet[i].ii * fpgaRecipeSet[i].clockMultiplier <= maxLatency && fpgaRecipeSet[i].area <= fpgaAreaRemaining - currentFpgaAreaUsed)
			{
	...
	if(!probeAllocate)
	{
		for(size_t i = 0; i < recipeSet.size(); i++)
		{
			...
			std::cout << "Allocating FPGA " << cfu.cfuID << " at port " << cfu.cpuID << " with mode " << cfu.acceleratorID << "  Consuming " << fpgaRecipeSet[recipeSet[i]].area << "/" << fpgaAreaRemaining << ", leaving " << (fpgaAreaRemaining - fpgaRecipeSet[recipeSet[i]].area) << std::endl;
...
void TD::DeallocateFpga(int id)
{
	assert(fpgaCfuPortMap.find(id) != fpgaCfuPortMap.end());
	assert(fpgaCfuRecipe.find(id) != fpgaCfuRecipe.end());
	ML_LOG(GetDeviceName(), "END FPGA Allocation " << id << " Mode " << fpgaCfuRecipe[id]->opCode << " II " << fpgaCfuRecipe[id]->ii << " Depth " << fpgaCfuRecipe[id]->pipelineDepth << " Clock " << fpgaCfuRecipe[id]->clockMultiplier);
	fpgaCfuPool[fpgaCfuPortMap[id]].push(id);
	fpgaAreaRemaining += fpgaCfuRecipe[id]->area;
	fpgaCfuRecipe.erase(id);
}
bool TD::IsFpgaCfu(int id)
{
	return fpgaCfuPortMap.find(id) != fpgaCfuPortMap.end();
}
void TD::MsgHandler(int src, const void* packet, unsigned int packetSize)
{
	assert(packetSize >= 8);
	uint32_t* msg = (uint32_t*)packet;
	int command = msg[0];
	uint32_t process = msg[1];
	for(unsigned int i = 0; i < packetSize / 4; i++)
	{
		std::cout << "[" << msg[i] << "]";
	}
	std::cout << std::endl;
	switch(command)
	{
		...
		case(LCACC_CMD_TASK_COMPLETED): //message from lca marking task as completed
		{
			...
			if(allDone)
			{
				...
				ML_LOG(GetDeviceName(), "END Program Execute " << userProcess);
...
std::cout << "Sending " << userProcess << " " << src << " translation for " << logicalPage << "(Address " << logicalAddr << ")";
assert(logicalPage);
			if(localTlb[userProcess].find(logicalPage) != localTlb[userProcess].end())
			{//service locally
std::cout << " being serviced locally" << std::endl;
                                uint32_t outMsg[9];
			...
			{//redundant miss
std::cout << " is redundant, and is queued up" << std::endl;
				cfuTlbMisses[userProcess][logicalPage].push_back(src);
			}
			else
			{//actual new miss
std::cout << " is an authentic miss, and is forwarded to cpu" << std::endl;
				cfuTlbMisses[userProcess][logicalPage].push_back(src);
				uint32_t outMsg[4];
...
std::cout << "TD:  process : " << process << "   Logical page " << logicalPage << " physical page " << physicalPage << std::endl;
			assert(physicalPage);
			assert(logicalPage);
			localTlb[process][logicalPage] = physicalPage;
			if(programSet.find(process) == programSet.end())//no program loaded, thus the miss must be on loading the program
			{
...
void TD::ExecuteProgram(TaskReadData& trd)
{
	...
	ML_LOG(GetDeviceName(), "BEGIN Program Execute " << trd.process);
	...
	if(tdp->taskGrain == 0)
	{
std::cout << "Setting task grain to default of " << defaultTaskGrain << std::endl;
		tdp->taskGrain = defaultTaskGrain;
	}
	requiredBufferSize[trd.process] = trd.sharedBufferSize;
	assert(pr.SizeRemaining() < 4);
std::cout << "tdp->taskCount " << tdp->taskCount << " tdp->taskGrain " << tdp->taskGrain << std::endl;
	uint32_t totalJobId = (tdp->taskCount + (tdp->taskGrain - 1)) / tdp->taskGrain;
std::cout << "TD: Total task count : " << tdp->taskCount << std::endl;
std::cout << "TD: Total job count : " << totalJobId << std::endl;
	assert(totalJobId > 0);
...
void TD::ReadProgramDone()
{
	assert(!pendingTaskReads.empty());
	TaskReadData& trd = pendingTaskReads.front();
	ML_LOG(GetDeviceName(), "END Program Read " << trd.process);
	assert(trd.loadedPacket == NULL);
...
std::cout << "TD Starting task execution immediately" << std::endl;
		ExecuteProgram(trd);
	}
	else
	{//We are already running the program, so queue it up, and we'll get it later
std::cout << "TD Adding task to pending list" << std::endl;
...
void TD::StartProgramRead()
{
	if(pendingTaskReads.size() == 0)
	{
		return;
	}
	if(currentlyReadingTask)
	{
std::cout << "Already reading a task." << std::endl;
		return;
	}
	currentlyReadingTask = true;
	TaskReadData& trd = pendingTaskReads.front();
	ML_LOG(GetDeviceName(), "BEGIN Program Read " << trd.process);
...
std::cout << "Reading program from " << trd.logicalAddr << " of size " << trd.size << std::endl;
	dma->BeginTransfer(NO_SPM_ID, trd.logicalAddr, size, stride, spm->GetID(), 0, size, stride, 1, ReadProgramDoneCB::Create(this));
}
void TD::HandleTLBMiss(uint64_t addr)
{
	uint32_t msg[4];
	assert(!pendingTaskReads.empty());
	TaskReadData& trd = pendingTaskReads.front();
	...
	msg[2] = bc.u32[0];
	msg[3] = bc.u32[1];
	ML_LOG(GetDeviceName(), "BEGIN Personal TLB miss " << bc.u64[0]);
	netPort->SendMessage(lastKnownCore[trd.process], msg, sizeof(msg));
}
void TD::HandleAccessViolation(uint64_t)
{
	std::cerr << "[ERROR] Access violation picked up in TD." << std::endl;
	assert(0);
}
void TD::AllocateMore()
{
	assert(currentlyAllocating);
	currentlyAllocating = false;
	TryNewAllocation();
}
void TD::FreeBuffer(int bufferID, bool silent)
{
	if(!silent)
	{
		assert(bufferNodes.find(bufferID) != bufferNodes.end());
		TaskDistributor::gem5Interface::FreeBuffer(bufferNodes[bufferID], bufferID, BufferFreeCompleteCB::Create(this, bufferID));
	}
	else
	{
...
void TD::BufferFreeComplete(int bufferID)
{
	assert(bufferNodes.find(bufferID) != bufferNodes.end());
	freeBuffers[bufferNodes[bufferID]].push_back(bufferID);
	bufferNodes.erase(bufferID);
}
int TD::AcquireBufferID(uint32_t node)
{
	if(freeBuffers[node].size() == 0)
	{
		freeBuffers[node].push_back(TaskDistributor::gem5Interface::GetBufferID(node));
	}
...
void TD::AllocateBufferComplete(bool success, int bufferSize, int bufferID, std::vector<CFUIdentifier> selected, unsigned int process)
{
	...
	if(!success)
	{
		...
		if(cfuUseMap.empty())
		{
			std::cout << "This buffer can NEVER be allocated.  Crashing...." << std::endl;
...
std::cout << "Configuring " << selected[x].cfuID << std::endl;
		...
		if(!it->second.empty())
		{
			...
			else
			{
				...
				for(size_t x = 0; x > fpgaOpCodeSummary.size(); x++)
				{
					...
					if(!foundSlower)
					{
						std::cout << "Asic speed: " << maxAsicLatency << "  for op code " << fpgaOpCodeSummary[x] << std::endl;
						assert(foundSlower);
					}
				}
...
void TD::AddNetworkInterface(NetworkInterface* port)
{
	assert(netPort == NULL);
	netPort = port;
}
void TD::AddCFU(int id, int cpu, int type)
{
...
void TD::SetSelector(const std::string& selectorName)
{
	assert(patternSelector);
...
#undef PS_LISTING
	assert(patternSelector);
	std::cout << "Pattern selector set to : " << selectorName << std::endl;
}
void TD::SetCFUAllocationPerIsland(int cfuCount)
{
	assert(cfuCount > 0);
	cfusPerIsland = cfuCount;
}
void TD::SetTaskGrain(int grain)
{
	defaultTaskGrain = grain;
}
void TD::SetFpgaArea(int area)
{
	assert(area >= 0);
	fpgaAreaRemaining = area;
}
void TD::AddFpgaPort(int networkPort)
{
	assert(fpgaPortLoad.find(networkPort) == fpgaPortLoad.end());
	fpgaPortLoad[networkPort] = 0;
}
void TD::AddFpgaRecipe(int opCode, int area, int ii, int pipelineDepth, int cycleMult)
{
	assert(ii > 0);
...
void TD::ClearCFUFilter(uint32_t process)
{
	assert(cfuUseFilter.find(process) != cfuUseFilter.end());
	cfuUseFilter.erase(process);
}
void TD::AddCFUFilter(uint32_t process, int cfuID)
{
