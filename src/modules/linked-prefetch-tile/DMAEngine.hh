...
public:
  ...
  TransferData(int srcSpmID, uint64_t srcLAddr, int dstSpmID, uint64_t dstLAddr,
    size_t elemSize, int prio, uint64_t when, int buffer, CallbackBase* onFinish);
...
public:
  void AddTransferSet(int srcDevice, uint64_t srcAddr,
  ...
  void PrefetchSet(uint64_t addr, unsigned int dimensions,
    const unsigned int* elementSize, const int* elementStride,
    size_t transferSize);

  void AddSingleTransfer(int srcDevice, uint64_t srcAddr, int dstDevice,
  ...
  void Configure(int node, int spxm, Arg1CallbackBase<uint64_t>* onTLBMiss,
    Arg1CallbackFunctionBase<uint64_t, uint64_t>* translateAddress,
    Arg1CallbackBase<uint64_t>* onError);
